...
StatusBars.prototype.SetEnabled = function(enabled, showRank) {
  ...
  if (enabled == this.enabled && showRank == this.showRank) return;
...
StatusBars.prototype.AddAuraSource = function(source, auraName) {
  if (this.auraSources.has(source)) this.auraSources.get(source).push(auraName);
  else this.auraSources.set(source, [auraName]);
  this.RegenerateSprites();
};
...
StatusBars.prototype.OnEntityFinanceChanged = function(msg) {
  if (this.enabled) this.RegenerateSprites();
};
...
StatusBars.prototype.UpdateColor = function() {
  if (this.usedPlayerColors) this.RegenerateSprites();
};

StatusBars.prototype.RegenerateSprites = function() {
  let cmpOverlayRenderer = Engine.QueryInterface(
    this.entity,
    IID_OverlayRenderer
  );
 ...
 */
StatusBars.prototype.AddBar = function(
  cmpOverlayRenderer,
  yoffset,
  type,
  amount
...
StatusBars.prototype.AddPackBar = function(cmpOverlayRenderer, yoffset) {
  ...
  if (!cmpPack || !cmpPack.IsPacking()) return 0;
...
StatusBars.prototype.AddHealthBar = function(cmpOverlayRenderer, yoffset) {
  ...
  if (!cmpHealth || cmpHealth.GetHitpoints() <= 0) return 0;
...
StatusBars.prototype.AddWalletBar = function(cmpOverlayRenderer, yoffset) {
  if (!this.enabled) return 0;

  let cmpWallet = QueryMiragedInterface(this.entity, IID_EntityFinance);
  if (!cmpWallet || cmpWallet.GetFinancialStatus().balance <= 0) return 0;
...
StatusBars.prototype.AddResourceSupplyBar = function(
  ...
  let value = cmpResourceSupply.IsInfinite()
    ? 1
    : cmpResourceSupply.GetCurrentAmount() / cmpResourceSupply.GetMaxAmount();
...
StatusBars.prototype.AddCaptureBar = function(cmpOverlayRenderer, yoffset) {
  ...
  if (!cmpCapturable) return 0;

  let cmpOwnership = QueryMiragedInterface(this.entity, IID_Ownership);
  if (!cmpOwnership) return 0;

  let owner = cmpOwnership.GetOwner();
  if (owner == INVALID_PLAYER) return 0;
  ...
  for (let i in cp)
    if (i != owner && cp[i] > 0) size = setCaptureBarPart(i, size);
...
StatusBars.prototype.AddAuraIcons = function(cmpOverlayRenderer, yoffset) {
  ...
  let sources = cmpGuiInterface
    .GetEntitiesWithStatusBars()
    .filter(e => this.auraSources.has(e) && this.auraSources.get(e).length);
...
StatusBars.prototype.AddRankIcon = function(cmpOverlayRenderer, yoffset) {
  if (!this.enabled || !this.showRank) return 0;

  let cmpIdentity = Engine.QueryInterface(this.entity, IID_Identity);
  if (!cmpIdentity || !cmpIdentity.GetRank()) return 0;
