...
UnitAI.prototype.UnitFsmSpec = {
    ...
    "Order.Shopping": function(msg) {
        ...
        let target = this.order.data.producerTargets.closerProducer
        //this.PerformShopping(this.order.data.producerTargets.closerProducer, false);
		...
		// Check if the target is in range
		if (this.CheckTargetRange(target, IID_EntityProducer))
		{
            //error("is in range")
			this.StopMoving();
			this.SetNextState("INDIVIDUAL.SHOPPING.SHOPPING");
			return;
		}
		...
		{
			// We've started walking to the given point
            //error("is not in range")
			this.SetNextState("INDIVIDUAL.SHOPPING.APPROACHING");
			return;
		}

		// We can't reach the target, and can't move towards it,
		// so abandon this shopping order
	...
	"INDIVIDUAL": {
				...
				"Timer": function(msg) {
                    ...
                    // If meantime we work we need to buy some goods
                            ...
                            if (cmpEntityConsumer) {
                              ...
                              let fCP = needToBuy.firstChoiceProduct
                              var cmpEntityFinance = Engine.QueryInterface(this.entity, IID_EntityFinance);
                              var status = cmpEntityFinance.GetFinancialStatus()
                              
                              if (needToBuy.firstChoiceProduct != null && status.balance/status.walletCapacity > 0.1) {
                                ...
                                var targetProducer = this.FindNearestProducer(firstChoiceProducerType,fCP);
                                // we record our last position
                                var lastPos = undefined;
                                var cmpPosition = Engine.QueryInterface(this.entity, IID_Position);
                                if (cmpPosition && cmpPosition.IsInWorld())
                                  lastPos = cmpPosition.GetPosition();
                                  let tar = this.order.data.target
                                  let typ = this.order.data.type
                                  let temp = this.order.data.template
        ...
        SHOPPING: {
            ...
            "APPROACHING": {
				...
				"MoveCompleted": function() {
					this.SetNextState("SHOPPING");
				},
			},
            "SHOPPING": {
				"enter": function() {
					
                    this.SelectAnimation("shopping");
                    this.StartTimer(1000, 1000);	
				...
				"Timer": function(msg) {
                    ...
                    var target = this.order.data.producerTargets.closerProducer
                    var needToBuy = this.order.data.needToBuy
                    var cmpProductManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_ProductsManager);
                    var cmpEntityConsumer = Engine.QueryInterface(this.entity, IID_EntityConsumer);
                    var cmpTargetProducer = Engine.QueryInterface(target, IID_EntityProducer);
                    var cmpEntityFinance = Engine.QueryInterface(this.entity, IID_EntityFinance);
                    var cmpTargetFinance = Engine.QueryInterface(target, IID_EntityFinance);
                    // we check how much money we got 
                    var balance = cmpEntityFinance.GetFinancialStatus().balance;
                    // we check how many product we need to buy
                    var totProduct = Object.keys(needToBuy).length;
                    // we split the balance
                    balance = balance/totProduct-1;
                    // we make the purchase
                    for (let type in needToBuy) {
                        let price = cmpProductManager.GetPrice(type);
                        let feasableOrder = balance/price;
                        let availability = cmpTargetProducer.GetProductAvailability(type);
                        let amount = 0
                        feasableOrder > availability ? amount =availability * price: amount =feasableOrder* price
                        let order = amount / price
                        order = Math.round(order);
                        cmpEntityFinance.Spend(amount);
                        cmpTargetProducer.MinusProductToCatalogue(type,order);
                        cmpEntityConsumer.AddProductCarring(type,order);
                        cmpTargetFinance.Earn(amount);
                        
                        
                    }
                    // and now we back to work
                    let tar = this.order.data.gatheringTarget
                    let typ = this.order.data.type
                    let tmp = this.order.data.template
                    let lp = this.order.data.lastPos
                    this.StopTimer();
                    this.FinishOrder();
                    this.PushOrderFront("Gather", {"target": tar ,"type":typ, "template":tmp , "lastpos":lp , "autoharvest":true, "force":true})
...
UnitAI.prototype.FindNearestProducer = function(TypeAry, productTypeObj) {
  let cmpOwnership = Engine.QueryInterface(this.entity, IID_Ownership);
  if (!cmpOwnership || cmpOwnership.GetOwner() == INVALID_PLAYER)
    return undefined;
  ...
  for (let producer of nearbyProducers) {
    ...
    for (let Type of TypeAry) {
      if (cmpEntityProducer.getType() == Type) {
        for (let Type2 in productTypeObj) {
          let amount = cmpEntityProducer.GetProductAvailability(Type2)
          if (amount < 20)
              continue
