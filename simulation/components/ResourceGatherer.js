...
ResourceGatherer.prototype.GiveResources = function(resources) {
  ...
  Engine.PostMessage(this.entity, MT_ResourceCarryingChanged, {
    to: this.GetCarryingStatus()
  });
};
...
ResourceGatherer.prototype.GetMainCarryingType = function() {
  ...
  for (let type in this.carrying) return type;
...
ResourceGatherer.prototype.RecalculateGatherRatesAndCapacities = function() {
  ...
  for (let r in this.template.Rates) {
    ...
    if (
      type[0] != "treasure" &&
      type.length > 1 &&
      !Resources.GetResource(type[0]).subtypes[type[1]]
    ) {
    ...
    let rate = ApplyValueModificationsToEntity(
      "ResourceGatherer/Rates/" + r,
      +this.template.Rates[r],
      this.entity
    );
  ...
  for (let r in this.template.Capacities)
    this.capacities[r] = ApplyValueModificationsToEntity(
      "ResourceGatherer/Capacities/" + r,
      +this.template.Capacities[r],
      this.entity
    );
};
...
ResourceGatherer.prototype.TryInstantGather = function(target) {
  ...
  let status = cmpResourceSupply.TakeResources(
    cmpResourceSupply.GetCurrentAmount()
  );

  let cmpPlayer = QueryOwnerInterface(this.entity, IID_Player);
  if (cmpPlayer) cmpPlayer.AddResource(type.specific, status.amount);
...
ResourceGatherer.prototype.PerformGather = function(target) {
  if (!this.GetTargetGatherRate(target)) return { exhausted: true };
  ...
  if (!this.carrying[type.generic]) this.carrying[type.generic] = 0;

  // Find the maximum so we won't exceed our capacity
  let maxGathered =
    this.GetCapacity(type.generic) - this.carrying[type.generic];

  let status = cmpResourceSupply.TakeResources(
    Math.min(gatherAmount, maxGathered)
  );
  if (type.generic == "food") {
    this.foodSubTypesCarrying[type.specific] += status.amount;
  }
  //error(this.foodSubTypesCarrying[type.specific]);

  this.carrying[type.generic] += status.amount;

  this.lastCarriedType = type;

  // Make it earn money for the job done if is a civilian, not military
  //let cmpEntityConsumer = Engine.QueryInterface(
  //  this.entity,
  //  IID_EntityConsumer
  //);
  //if (cmpEntityConsumer) this.getPay(status.amount);
  ...
  if (cmpStatisticsTracker)
    cmpStatisticsTracker.IncreaseResourceGatheredCounter(
      type.generic,
      status.amount,
      type.specific
    );
...
ResourceGatherer.prototype.GetTargetGatherRate = function(target) {
  ...
  if (rate == 0 && type.generic) rate = this.GetGatherRate(type.generic);

  if ("Mirages" in cmpResourceSupply) return rate;
  ...
  if (diminishingReturns) rate *= diminishingReturns;
...
ResourceGatherer.prototype.IsCarryingAnythingExcept = function(exceptedType) {
  for (let type in this.carrying) if (type != exceptedType) return true;
...
ResourceGatherer.prototype.getPay = function(amount) {
  let cmpEntityFinance = Engine.QueryInterface(this.entity, IID_EntityFinance);
  cmpEntityFinance.Earn(amount);
  //error(cmpEntityFinance.GetFinancialStatus().balance);
};
...
ResourceGatherer.prototype.OnValueModification = function(msg) {
  if (msg.component != "ResourceGatherer") return;
  this.RecalculateGatherRatesAndCapacities();
};

ResourceGatherer.prototype.OnOwnershipChanged = function(msg) {
  if (msg.to == INVALID_PLAYER) return;
  this.RecalculateGatherRatesAndCapacities();
};
