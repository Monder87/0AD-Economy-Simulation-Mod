...
ResourceSupply.prototype.Init = function() {
  ...
  for (let i = 0; i < numPlayers; ++i) this.gatherers.push([]);
...
ResourceSupply.prototype.GetDiminishingReturns = function() {
  if ("DiminishingReturns" in this.template) {
    let diminishingReturns = ApplyValueModificationsToEntity(
      "ResourceSupply/DiminishingReturns",
      +this.template.DiminishingReturns,
      this.entity
    );
    if (diminishingReturns) {
      let numGatherers = this.GetNumGatherers();
      if (numGatherers > 1)
        return diminishingReturns == 1
          ? 1
          : (1 - Math.pow(diminishingReturns, numGatherers)) /
              (1 - diminishingReturns) /
              numGatherers;
    }
  }
  return null;
...
ResourceSupply.prototype.TakeResources = function(rate) {
  ...
  if (this.infinite) return { amount: rate, exhausted: false };
  ...
  if (this.amount === 0) Engine.DestroyEntity(this.entity);
