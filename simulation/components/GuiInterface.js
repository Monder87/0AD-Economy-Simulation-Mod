...
GuiInterface.prototype.Schema = "<a:component type='system'/><empty/>";
...
GuiInterface.prototype.GetSimulationState = function() {
  ...
  for (let i = 0; i < numPlayers; ++i) {
    ...
    ret.players.push({
      ...
      isNeutral: neutrals,
      isEnemy: enemies,
      entityLimits: cmpPlayerEntityLimits
        ? cmpPlayerEntityLimits.GetLimits()
        : null,
      entityCounts: cmpPlayerEntityLimits
        ? cmpPlayerEntityLimits.GetCounts()
        : null,
      entityLimitChangers: cmpPlayerEntityLimits
        ? cmpPlayerEntityLimits.GetLimitChangers()
        : null,
      researchQueued: cmpTechnologyManager
        ? cmpTechnologyManager.GetQueuedResearch()
        : null,
      researchStarted: cmpTechnologyManager
        ? cmpTechnologyManager.GetStartedTechs()
        : null,
      researchedTechs: cmpTechnologyManager
        ? cmpTechnologyManager.GetResearchedTechs()
        : null,
      classCounts: cmpTechnologyManager
        ? cmpTechnologyManager.GetClassCounts()
        : null,
      typeCountsByClass: cmpTechnologyManager
        ? cmpTechnologyManager.GetTypeCountsByClass()
        : null,
  ...
  if (cmpRangeManager) ret.circularMap = cmpRangeManager.GetLosCircular();

  let cmpTerrain = Engine.QueryInterface(SYSTEM_ENTITY, IID_Terrain);
  if (cmpTerrain) ret.mapSize = cmpTerrain.GetMapSize();
  ...
  // Add ceasefire info
  let cmpCeasefireManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_CeasefireManager
  );
  if (cmpCeasefireManager) {
    ret.ceasefireActive = cmpCeasefireManager.IsCeasefireActive();
    ret.ceasefireTimeRemaining = ret.ceasefireActive
      ? cmpCeasefireManager.GetCeasefireStartedTime() +
        cmpCeasefireManager.GetCeasefireTime() -
        ret.timeElapsed
      : 0;
  }

  // Add cinema path info
  let cmpCinemaManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_CinemaManager
  );
  if (cmpCinemaManager) ret.cinemaPlaying = cmpCinemaManager.IsPlaying();

  // Add the game type and allied victory
  let cmpEndGameManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_EndGameManager
  );
  ...
  for (let i = 0; i < numPlayers; ++i) {
    let cmpPlayerStatisticsTracker = QueryPlayerIDInterface(
      i,
    ...
    if (cmpPlayerStatisticsTracker)
      ret.players[
        i
      ].statistics = cmpPlayerStatisticsTracker.GetBasicStatistics();
  }
...
GuiInterface.prototype.AddMiragedEntity = function(player, entity, mirage) {
  if (!this.miragedEntities[player]) this.miragedEntities[player] = [];
...
GuiInterface.prototype.GetEntityState = function(player, ent) {
  ...
  if (cmpHealth) {
    ...
    ret.needsRepair =
      cmpHealth.IsRepairable() &&
      cmpHealth.GetHitpoints() < cmpHealth.GetMaxHitpoints();
    ret.needsHeal = !cmpHealth.IsUnhealable();
  }

  let cmpWallet = QueryMiragedInterface(ent, IID_EntityFinance);
  if (cmpWallet) {
    ret.balance = cmpWallet.GetFinancialStatus().balance;
    ret.walletMaxCapacity = cmpWallet.GetFinancialStatus().walletCapacity;
  }
  ...
  if (cmpOwnership) ret.player = cmpOwnership.GetOwner();
  ...
  if (cmpAlertRaiser) ret.alertRaiser = true;
  ...
  if (cmpArmour) ret.armour = cmpArmour.GetArmourStrengths();
...
GuiInterface.prototype.GetMultipleEntityStates = function(player, ents) {
  return ents.map(ent => ({
    entId: ent,
    state: this.GetEntityState(player, ent)
  }));
};
...
GuiInterface.prototype.GetAverageRangeForBuildings = function(player, cmd) {
  ...
  return cmpRangeManager.GetElevationAdaptedRange(
    pos,
    rot,
    range,
    elevationBonus,
...
GuiInterface.prototype.GetTemplateData = function(player, templateName) {
  ...
  for (let name of auraNames) aurasTemplate[name] = AuraTemplates.Get(name);
...
GuiInterface.prototype.IsTechnologyResearched = function(player, data) {
  if (!data.tech) return true;

  let cmpTechnologyManager = QueryPlayerIDInterface(
    data.player || player,
    IID_TechnologyManager
  );

  if (!cmpTechnologyManager) return false;
...
GuiInterface.prototype.GetBattleState = function(player) {
  ...
  if (!cmpBattleDetection) return false;
...
GuiInterface.prototype.GetNeededResources = function(player, data) {
  return QueryPlayerIDInterface(data.player || player).GetNeededResources(
    data.cost
  );
};
 ...
 */
GuiInterface.prototype.AddTimeNotification = function(
  notification,
  duration = 10000
) {
  let cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
  ...
  if (!notification.players) {
    notification.players = Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_PlayerManager
    ).GetAllPlayers();
...
GuiInterface.prototype.DeleteTimeNotification = function(notificationID) {
  this.timeNotifications = this.timeNotifications.filter(
...
GuiInterface.prototype.GetTimeNotifications = function(player) {
  ...
  return this.timeNotifications.filter(
    n => n.players.indexOf(player) != -1 && n.endTime > time
  );
};
...
GuiInterface.prototype.GetFormationInfoFromTemplate = function(player, data) {
  ...
  if (!template || !template.Formation) return {};
...
GuiInterface.prototype.GetStockInfoFromTemplate = function(player, data) {
  //error("ciaooo");
  let cmpTemplateManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_TemplateManager
  ...
  let template = cmpTemplateManager.GetTemplate(data.templateName);
  //error(template.Formation.FormationName);
  if (!template || !template.Formation) return {};
...
GuiInterface.prototype.IsFormationSelected = function(player, data) {
  return data.ents.some(ent => {
    let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
    return (
      cmpUnitAI && cmpUnitAI.GetFormationTemplate() == data.formationTemplate
    );
  });
};
...
GuiInterface.prototype.IsStanceSelected = function(player, data) {
  for (let ent of data.ents) {
    let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
    if (cmpUnitAI && cmpUnitAI.GetStanceName() == data.stance) return true;
  }
  return false;
};
...
GuiInterface.prototype.RightPanelFocused = function(player, panelName) {
  //error(panelName + "ciao" + this.panelTracker);

  this.panelTracker = panelName;
};

GuiInterface.prototype.RightPanelEnabled = function(player, panelName) {
  if (this.panelTracker == panelName) {
    return true;
  } else {
    return false;
  }
...
GuiInterface.prototype.GetAllConsumingProducts = function(player, cmd) {
  let consumingProducts = [];
  for (let ent of cmd.entities) {
    let cmpEntityConsumer = Engine.QueryInterface(ent, IID_EntityConsumer);
    if (!cmpEntityConsumer) continue;

    for (let product of cmpEntityConsumer.GetProductTypes()) {
      error(product);
      if (consumingProducts.indexOf(product) == -1)
        consumingProducts.push(product);
    }
  }
  return consumingProducts;
};

GuiInterface.prototype.GetAvailableStock = function(ent) {
  return [
    "special/stock/sausage",
    "special/stock/bread",
    "special/stock/steak",
    "special/stock/wine",
    "special/stock/clothes",
    "special/stock/entertainment",
    "special/stock/jewelry",
    "special/stock/slave"
  ];
};
...
GuiInterface.prototype.UpdateDisplayedPlayerColors = function(player, data) {
  ...
  for (let i = 1; i < numPlayers; ++i) {
    ...
    updateEntityColor(
      data.showAllStatusBars && (i == player || player == -1)
        ? [
            IID_Minimap,
...
GuiInterface.prototype.SetSelectionHighlight = function(player, cmd) {
  ...
  for (let ent of cmd.entities) {
    ...
    if (!cmpSelectable) continue;

    // Find the entity's owner's color:
    let owner = INVALID_PLAYER;
    let cmpOwnership = Engine.QueryInterface(ent, IID_Ownership);
    if (cmpOwnership) owner = cmpOwnership.GetOwner();

    let color = playerColors[owner];
    if (!color) {
      color = { r: 1, g: 1, b: 1 };
      let cmpPlayer = QueryPlayerIDInterface(owner);
      if (cmpPlayer) color = cmpPlayer.GetDisplayedColor();
      playerColors[owner] = color;
    }

    cmpSelectable.SetSelectionHighlight(
      { r: color.r, g: color.g, b: color.b, a: cmd.alpha },
      cmd.selected
    );

    let cmpRangeOverlayManager = Engine.QueryInterface(
      ent,
      IID_RangeOverlayManager
    );
    if (
      !cmpRangeOverlayManager ||
      (player != owner && player != INVALID_PLAYER)
    )
      continue;

    cmpRangeOverlayManager.SetEnabled(
      cmd.selected,
      this.enabledVisualRangeOverlayTypes,
      false
    );
...
GuiInterface.prototype.SetStatusBars = function(player, cmd) {
  ...
  for (let ent of cmd.entities) {
    ...
    if (!cmpStatusBars) continue;
    cmpStatusBars.SetEnabled(cmd.enabled, cmd.showRank);

    let cmpAuras = Engine.QueryInterface(ent, IID_Auras);
    if (!cmpAuras) continue;

    for (let name of cmpAuras.GetAuraNames()) {
      if (!cmpAuras.GetOverlayIcon(name)) continue;
      for (let e of cmpAuras.GetAffectedEntities(name)) affectedEnts.add(e);
      if (cmd.enabled) this.entsWithAuraAndStatusBars.add(ent);
      else this.entsWithAuraAndStatusBars.delete(ent);
    }
  }

  for (let ent of affectedEnts) {
    let cmpStatusBars = Engine.QueryInterface(ent, IID_StatusBars);
    if (cmpStatusBars) cmpStatusBars.RegenerateSprites();
  }
};
...
GuiInterface.prototype.DisplayRallyPoint = function(player, cmd) {
  ...
  for (let ent of this.entsRallyPointsDisplayed) {
    let cmpRallyPointRenderer = Engine.QueryInterface(
      ent,
      IID_RallyPointRenderer
    );
    if (cmpRallyPointRenderer) cmpRallyPointRenderer.SetDisplayed(false);
  }
  ...
  for (let ent of cmd.entities) {
    ...
    if (!cmpRallyPointRenderer) continue;

    // entity must have a rally point component to display a rally point marker
    // (regardless of whether cmd specifies a custom location)
    let cmpRallyPoint = Engine.QueryInterface(ent, IID_RallyPoint);
    if (!cmpRallyPoint) continue;

    // Verify the owner
    let cmpOwnership = Engine.QueryInterface(ent, IID_Ownership);
    if (!(cmpPlayer && cmpPlayer.CanControlAllUnits()))
      if (!cmpOwnership || cmpOwnership.GetOwner() != player) continue;

    // If the command was passed an explicit position, use that and
    // override the real rally point position; otherwise use the real position
    let pos;
    if (cmd.x && cmd.z) pos = cmd;
    else pos = cmpRallyPoint.GetPositions()[0]; // may return undefined if no rally point is set
    ...
    if (pos) {
      ...
      if ("queued" in cmd)
        ...
        else cmpRallyPointRenderer.SetPosition({ x: pos.x, y: pos.z });
...
GuiInterface.prototype.SetBuildingPlacementPreview = function(player, cmd) {
  ...
  if (!this.placementEntity || this.placementEntity[0] != cmd.template) {
    ...
    if (this.placementEntity) Engine.DestroyEntity(this.placementEntity[1]);

    // Load the new template
    if (cmd.template == "") this.placementEntity = undefined;
    else
      this.placementEntity = [
        cmd.template,
        Engine.AddLocalEntity("preview|" + cmd.template)
      ];
  }
  ...
  if (this.placementEntity) {
    ...
    let cmpBuildRestrictions = Engine.QueryInterface(
      ent,
      IID_BuildRestrictions
    );
    if (!cmpBuildRestrictions) error("cmpBuildRestrictions not defined");
    else result = cmpBuildRestrictions.CheckPlacement();
    ...
    if (cmpVisual) {
      if (cmd.actorSeed !== undefined) cmpVisual.SetActorSeed(cmd.actorSeed);

      if (!result.success) cmpVisual.SetShadingColor(1.4, 0.4, 0.4, 1);
      else cmpVisual.SetShadingColor(1, 1, 1, 1);
    }
  }
...
GuiInterface.prototype.SetWallPlacementPreview = function(player, cmd) {
  ...
  if (!this.placementWallEntities) this.placementWallEntities = {};
  ...
  for (let type in wallSet.templates) {
    ...
    if (!(tpl in this.placementWallEntities)) {
      ...
      if (!this.placementWallEntities[tpl].templateData.wallPiece) {
        error(
          "[SetWallPlacementPreview] No WallPiece component found for wall set template '" +
            tpl +
            "'"
        );
        return false;
  ...
  if (cmd.snapEntities) {
    let snapRadius =
      this.placementWallEntities[wallSet.templates.tower].templateData.wallPiece
        .length * 0.5; // determined through trial and error
    ...
    if (startSnapData) {
      ...
      if (startSnapData.ent) start.snappedEnt = startSnapData.ent;
    }
    ...
    if (end.pos) {
      ...
      if (endSnapData) {
        ...
        if (endSnapData.ent) end.snappedEnt = endSnapData.ent;
      }
    }
  }
  ...
  for (let res of Resources.GetCodes()) result.cost[res] = 0;
  ...
  if (start.snappedEnt && start.snappedEnt != INVALID_ENTITY) {
    let startEntObstruction = Engine.QueryInterface(
      start.snappedEnt,
      IID_Obstruction
    );
    if (previewEntities.length > 0 && startEntObstruction)
      previewEntities[0].controlGroups = [
        startEntObstruction.GetControlGroup()
      ];
    ...
    previewEntities.unshift({
      ...
      angle:
        previewEntities.length > 0
          ? previewEntities[0].angle
          : this.placementWallLastAngle
    });
  }
  ...
  if (end.pos) {
    ...
    if (end.snappedEnt && end.snappedEnt != INVALID_ENTITY) {
      let endEntObstruction = Engine.QueryInterface(
        end.snappedEnt,
        IID_Obstruction
      );
      ...
      if (previewEntities.length > 0 && endEntObstruction) {
        previewEntities[previewEntities.length - 1].controlGroups =
          previewEntities[previewEntities.length - 1].controlGroups || [];
        previewEntities[previewEntities.length - 1].controlGroups.push(
          endEntObstruction.GetControlGroup()
        );
      }
      ...
      previewEntities.push({
        ...
        angle:
          previewEntities.length > 0
            ? previewEntities[previewEntities.length - 1].angle
            : this.placementWallLastAngle
      });
  }
  ...
  for (let i = 0; i < previewEntities.length; ++i) {
    ...
    if (!ent) {
      error(
        "[SetWallPlacementPreview] Failed to allocate or reuse preview entity of template '" +
          tpl +
          "'"
      );
      continue;
    ...
    if (cmpPosition) {
      ...
      if (tpl === wallSet.templates.tower) {
        ...
        if (i > 0)
          terrainGroundPrev = cmpTerrain.GetGroundLevel(
            previewEntities[i - 1].pos.x,
            previewEntities[i - 1].pos.z
          );

        if (i < previewEntities.length - 1)
          terrainGroundNext = cmpTerrain.GetGroundLevel(
            previewEntities[i + 1].pos.x,
            previewEntities[i + 1].pos.z
          );
    ...
    if (!cmpObstruction) {
      error(
        "[SetWallPlacementPreview] Preview entity of template '" +
          tpl +
          "' does not have an Obstruction component"
      );
      continue;
    }
    ...
    if (entInfo.controlGroups && entInfo.controlGroups.length > 0) {
      if (entInfo.controlGroups.length > 2) {
        error(
          "[SetWallPlacementPreview] Encountered preview entity of template '" +
            tpl +
            "' with more than 2 initial control groups"
        );
        break;
      }
    ...
    if (visible) {
      ...
      if (!cmpBuildRestrictions) {
        error(
          "[SetWallPlacementPreview] cmpBuildRestrictions not defined for preview entity of template '" +
            tpl +
            "'"
        );
        continue;
      ...
      validPlacement =
        cmpBuildRestrictions && cmpBuildRestrictions.CheckPlacement().success;
      ...
      if (
        validPlacement &&
        entInfo.controlGroups &&
        entInfo.controlGroups.length > 1
      )
    ...
    if (!entInfo.excludeFromResult) ++numRequiredPieces;
    ...
    if (allPiecesValid && !entInfo.excludeFromResult) {
      ...
      for (let res of Resources.GetCodes().concat([
        "population",
        "populationBonus",
        "time"
      ]))
  ...
  if (numRequiredPieces > 0 && result.pieces.length == 0) return false;
...
GuiInterface.prototype.GetFoundationSnapData = function(player, data) {
  ...
  if (!template) {
    warn(
      "[GetFoundationSnapData] Failed to load template '" + data.template + "'"
    );
    return false;
  }
  ...
  if (data.snapEntities && data.snapRadius && data.snapRadius > 0) {
    ...
    for (let ent of data.snapEntities) {
      let cmpPosition = Engine.QueryInterface(ent, IID_Position);
      if (!cmpPosition || !cmpPosition.IsInWorld()) continue;

      let pos = cmpPosition.GetPosition();
      let dist2 =
        (data.x - pos.x) * (data.x - pos.x) +
        (data.z - pos.z) * (data.z - pos.z);
    ...
    if (minDistEntitySnapData != null) return minDistEntitySnapData;
  }
...
GuiInterface.prototype.FindIdleUnits = function(player, data) {
  ...
  for (let entity of cmpRangeManager.GetEntitiesByPlayer(player)) {
    let filtered = this.IdleUnitFilter(
      entity,
      data.idleClasses,
      data.excludeUnits
    );
    if (!filtered.idle) continue;
    ...
    if (!idleUnits[bucket]) idleUnits[bucket] = [];
...
GuiInterface.prototype.HasIdleUnits = function(player, data) {
  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  return cmpRangeManager
    .GetEntitiesByPlayer(player)
    .some(
      unit =>
        this.IdleUnitFilter(unit, data.idleClasses, data.excludeUnits).idle
    );
};
 ...
 */
GuiInterface.prototype.IdleUnitFilter = function(
  unit,
  idleClasses,
  excludeUnits
) {
  ...
  if (!cmpIdentity) return { idle: false };

  let bucket = idleClasses.findIndex(elem =>
    MatchesClassList(cmpIdentity.GetClassesList(), elem)
  );
  if (bucket == -1 || excludeUnits.indexOf(unit) > -1) return { idle: false };

  return { idle: true, bucket: bucket };
};

GuiInterface.prototype.GetTradingRouteGain = function(player, data) {
  if (!data.firstMarket || !data.secondMarket) return null;

  return CalculateTraderGain(
    data.firstMarket,
    data.secondMarket,
    data.template
  );
...
GuiInterface.prototype.GetTradingDetails = function(player, data) {
  ...
  if (!cmpEntityTrader || !cmpEntityTrader.CanTrade(data.target)) return null;
...
GuiInterface.prototype.GetBatchTime = function(player, data) {
  ...
  if (!cmpProductionQueue) return 0;

  return cmpProductionQueue.GetBatchTime(data.batchSize);
};

GuiInterface.prototype.IsMapRevealed = function(player) {
  return Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager).GetLosRevealAll(
    player
  );
};
...
GuiInterface.prototype.SetPathfinderHierDebugOverlay = function(
  player,
  enabled
) {
  Engine.QueryInterface(SYSTEM_ENTITY, IID_Pathfinder).SetHierDebugOverlay(
    enabled
  );
};

GuiInterface.prototype.SetObstructionDebugOverlay = function(player, enabled) {
  Engine.QueryInterface(SYSTEM_ENTITY, IID_ObstructionManager).SetDebugOverlay(
    enabled
  );
};
...
GuiInterface.prototype.SetMotionDebugOverlay = function(player, data) {
  for (let ent of data.entities) {
    let cmpUnitMotion = Engine.QueryInterface(ent, IID_UnitMotion);
    if (cmpUnitMotion) cmpUnitMotion.SetDebugOverlay(data.enabled);
  }
};

GuiInterface.prototype.SetRangeDebugOverlay = function(player, enabled) {
  Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager).SetDebugOverlay(
    enabled
  );
};
...
GuiInterface.prototype.GetTraderNumber = function(player) {
  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  let traders = cmpRangeManager
    .GetEntitiesByPlayer(player)
    .filter(e => Engine.QueryInterface(e, IID_Trader));
  ...
  for (let ent of traders) {
    let cmpIdentity = Engine.QueryInterface(ent, IID_Identity);
    let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
    if (!cmpIdentity || !cmpUnitAI) continue;
    ...
    if (cmpIdentity.HasClass("Ship")) {
      ...
      if (cmpUnitAI.order && cmpUnitAI.order.type == "Garrison") {
        ...
        if (
          cmpHolderUnitAI &&
          cmpHolderUnitAI.order &&
          cmpHolderUnitAI.order.type == "Trade"
        )
...
GuiInterface.prototype.GetStartedMoneySupply = function() {
  ...
  for (let i = 0; i < numPlayers; ++i) {
    // Calculate GDP as a sum o all other resources for the price of each one
    // to calculate the price of each resource we need to calculate demand and offer, so the resources more used has a price lower cause more offer.
    // of course collecting resources not used has costs so each resources if not used has a expense. more use less expenses
    // The final gain would be the tax applied to all the gdp
    // let gdp = allResources * allPrice
    let cmpPlayer = QueryPlayerIDInterface(i);
...
let exposedFunctions = {
  ...
  GetStartedMoneySupply: 1
};

GuiInterface.prototype.ScriptCall = function(player, name, args) {
  if (exposedFunctions[name]) return this[name](player, args);
