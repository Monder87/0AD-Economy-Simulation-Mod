...
Player.prototype.Serialize = function() {
  let state = {};
  for (let key in this) if (this.hasOwnProperty(key)) state[key] = this[key];
...
Player.prototype.SetGatherRateMultiplier = function(value) {
  ...
  Engine.BroadcastMessage(MT_MultiplierChanged, {
    player: this.playerID,
    type: "gather"
  });
};
...
///----- FINANCE ----- \\\
Player.prototype.GetEarningRateMultiplier = function() {
  return this.earningRateMultiplier / this.cheatTimeMultiplier;
};

Player.prototype.SetEarningRateMultiplier = function(value) {
  this.earningRateMultiplier = value;
  Engine.BroadcastMessage(MT_MultiplierChanged, {
    player: this.playerID,
    type: "earn"
  });
...
Player.prototype.GetNeededResources = function(amounts) {
  ...
  for (var type in amounts)
    if (
      this.resourceCount[type] != undefined &&
      amounts[type] > this.resourceCount[type]
    )
      amountsNeeded[type] =
        amounts[type] - Math.floor(this.resourceCount[type]);

  if (Object.keys(amountsNeeded).length == 0) return undefined;
...
Player.prototype.SubtractResourcesOrNotify = function(amounts) {
  ...
  if (amountsNeeded) {
    ...
    if (i < 1) warn("Amounts needed but no amounts given?");
    else if (i == 1)
      msg = markForTranslation(
        "Insufficient resources - %(resourceAmount1)s %(resourceType1)s"
      );
    else if (i == 2)
      msg = markForTranslation(
        "Insufficient resources - %(resourceAmount1)s %(resourceType1)s, %(resourceAmount2)s %(resourceType2)s"
      );
    else if (i == 3)
      msg = markForTranslation(
        "Insufficient resources - %(resourceAmount1)s %(resourceType1)s, %(resourceAmount2)s %(resourceType2)s, %(resourceAmount3)s %(resourceType3)s"
      );
    else if (i == 4)
      msg = markForTranslation(
        "Insufficient resources - %(resourceAmount1)s %(resourceType1)s, %(resourceAmount2)s %(resourceType2)s, %(resourceAmount3)s %(resourceType3)s, %(resourceAmount4)s %(resourceType4)s"
      );
    else
      warn("Localisation: Strings are not localised for more than 4 resources");

    // Send as time-notification
    let cmpGUIInterface = Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_GuiInterface
    );
...
Player.prototype.TrySubtractResources = function(amounts) {
  if (!this.SubtractResourcesOrNotify(amounts)) return false;

  var cmpStatisticsTracker = QueryPlayerIDInterface(
    this.playerID,
...
Player.prototype.GetNextTradingGoods = function() {
  ...
  for (var i = 0; i < last; ++i) {
    ...
    if (value < sumProba) return this.tradingGoods[i].goods;
  }
...
Player.prototype.SetState = function(newState, message) {
  ...
  if (won) cmpRangeManager.SetLosRevealAll(this.playerID, true);
  else {
...
Player.prototype.SetTeam = function(team) {
  ...
  if (this.team != -1) {
    ...
    for (let i = 0; i < numPlayers; ++i) {
      ...
      if (this.team != cmpPlayer.GetTeam()) continue;
...
Player.prototype.SetDiplomacyIndex = function(idx, value) {
  ...
  if (this.state != "active" || cmpPlayer.state != "active") return;
...
Player.prototype.UpdateSharedLos = function() {
  let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
  let cmpTechnologyManager = Engine.QueryInterface(
    this.entity,
    IID_TechnologyManager
  );
  if (!cmpRangeManager || !cmpTechnologyManager) return;

  if (
    !cmpTechnologyManager.IsTechnologyResearched(this.template.SharedLosTech)
  ) {
...
Player.prototype.OnGlobalOwnershipChanged = function(msg) {
  if (msg.from != this.playerID && msg.to != this.playerID) return;

  var cmpIdentity = Engine.QueryInterface(msg.entity, IID_Identity);
  var cmpCost = Engine.QueryInterface(msg.entity, IID_Cost);

  if (msg.from == this.playerID) {
    if (cmpCost) this.popUsed -= cmpCost.GetPopCost();

    if (
      cmpIdentity &&
      MatchesClassList(cmpIdentity.GetClassesList(), panelEntityClasses)
    ) {
      let index = this.panelEntities.indexOf(msg.entity);
      if (index >= 0) this.panelEntities.splice(index, 1);
    }
  }
...
Player.prototype.OnResearchFinished = function(msg) {
  if (msg.tech == this.template.SharedLosTech) this.UpdateSharedLos();
...
Player.prototype.OnValueModification = function(msg) {
  if (msg.component != "Player") return;

  if (msg.valueNames.indexOf("Player/SpyCostMultiplier") != -1)
    this.spyCostMultiplier = ApplyValueModificationsToEntity(
      "Player/SpyCostMultiplier",
      +this.template.SpyCostMultiplier,
      this.entity
    );

  if (msg.valueNames.some(mod => mod.startsWith("Player/BarterMultiplier/")))
    for (let res in this.template.BarterMultiplier.Buy) {
      this.barterMultiplier.buy[res] = ApplyValueModificationsToEntity(
        "Player/BarterMultiplier/Buy/" + res,
        +this.template.BarterMultiplier.Buy[res],
        this.entity
      );
      this.barterMultiplier.sell[res] = ApplyValueModificationsToEntity(
        "Player/BarterMultiplier/Sell/" + res,
        +this.template.BarterMultiplier.Sell[res],
        this.entity
      );
    }
};
...
Player.prototype.TributeResource = function(player, amounts) {
  ...
  for (let resCode in amounts)
    if (
      Resources.GetCodes().indexOf(resCode) == -1 ||
  ...
  if (!this.SubtractResourcesOrNotify(amounts)) return;
  cmpPlayer.AddResources(amounts);

  var total = Object.keys(amounts).reduce(
    (sum, type) => sum + amounts[type],
    0
  );
  var cmpOurStatisticsTracker = QueryPlayerIDInterface(
    this.playerID,
    IID_StatisticsTracker
  );
  if (cmpOurStatisticsTracker)
    cmpOurStatisticsTracker.IncreaseTributesSentCounter(total);
  var cmpTheirStatisticsTracker = QueryPlayerIDInterface(
    player,
    IID_StatisticsTracker
  );
...
Player.prototype.SetDisabledTemplates = function(templates) {
  this.disabledTemplates = {};
  for (let template of templates) this.disabledTemplates[template] = true;
  Engine.BroadcastMessage(MT_DisabledTemplatesChanged, {});
...
Player.prototype.SetDisabledTechnologies = function(techs) {
  this.disabledTechnologies = {};
  for (let tech of techs) this.disabledTechnologies[tech] = true;
  Engine.BroadcastMessage(MT_DisabledTechnologiesChanged, {});
};
