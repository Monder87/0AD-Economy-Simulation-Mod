...
Health.prototype.Init = function() {
  ...
  this.regenRate = ApplyValueModificationsToEntity(
    "Health/RegenRate",
    +this.template.RegenRate,
    this.entity
  );
  this.idleRegenRate = ApplyValueModificationsToEntity(
    "Health/IdleRegenRate",
    +this.template.IdleRegenRate,
    this.entity
  );
...
Health.prototype.SetHitpoints = function(value) {
  ...
  if (cmpRangeManager) {
    ...
    else cmpRangeManager.SetEntityFlag(this.entity, "injured", false);
  }
...
Health.prototype.ExecuteRegeneration = function() {
  ...
  if (this.GetIdleRegenRate() != 0) {
    ...
    if (
      cmpUnitAI &&
      (cmpUnitAI.IsIdle() ||
        (cmpUnitAI.IsGarrisoned() && !cmpUnitAI.IsTurret()))
    )
      regen += this.GetIdleRegenRate();
  }

  if (regen > 0) this.Increase(regen);
  else this.Reduce(-regen);
};
...
Health.prototype.CheckRegenTimer = function() {
  ...
  if (
    (this.GetRegenRate() == 0 && this.GetIdleRegenRate() == 0) ||
    (this.GetHitpoints() == this.GetMaxHitpoints() &&
      this.GetRegenRate() >= 0 &&
      this.GetIdleRegenRate() >= 0) ||
    this.GetHitpoints() == 0
  ) {
  ...
  var cmpTimer = Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
  this.regenTimer = cmpTimer.SetInterval(
    this.entity,
    IID_Health,
    "ExecuteRegeneration",
    1000,
...
Health.prototype.Reduce = function(amount) {
  ...
  if (amount >= 0 && this.hitpoints == this.GetMaxHitpoints()) {
    var cmpRangeManager = Engine.QueryInterface(
      SYSTEM_ENTITY,
      IID_RangeManager
    );
    if (cmpRangeManager)
  ...
  if (amount >= this.hitpoints) {
    ...
    if (this.hitpoints) {
      ...
      if (cmpDeathDamage) cmpDeathDamage.CauseDeathDamage();

      PlaySound("death", this.entity);

      if (this.template.SpawnEntityOnDeath) this.CreateDeathSpawnedEntity();
...
Health.prototype.Increase = function(amount) {
  ...
  // If we're already dead, don't allow resurrection
  if (this.hitpoints == 0) return undefined;
...
Health.prototype.CreateCorpse = function(leaveResources) {
  ...
  if (!cmpPosition.IsInWorld()) return INVALID_ENTITY;
  ...
  var cmpTemplateManager = Engine.QueryInterface(
    SYSTEM_ENTITY,
    IID_TemplateManager
  );
  var templateName = cmpTemplateManager.GetCurrentTemplateName(this.entity);
  var corpse;
  if (leaveResources) corpse = Engine.AddEntity("resource|" + templateName);
  else corpse = Engine.AddLocalEntity("corpse|" + templateName);
...
Health.prototype.UpdateActor = function() {
  if (!this.template.DamageVariants) return;
  ...
  if (ratio > 0) {
    ...
    for (let key in this.template.DamageVariants) {
      ...
      if (treshold < ratio || treshold > minTreshold) continue;
      newDamageVariant = key;
      minTreshold = treshold;
    }
  } else newDamageVariant = "death";

  if (this.damageVariant && this.damageVariant == newDamageVariant) return;

  this.damageVariant = newDamageVariant;

  let cmpVisual = Engine.QueryInterface(this.entity, IID_Visual);
  if (cmpVisual) cmpVisual.SetVariant("health", newDamageVariant);
};

Health.prototype.OnValueModification = function(msg) {
  if (msg.component != "Health") return;

  let oldMaxHitpoints = this.GetMaxHitpoints();
  let newMaxHitpoints = ApplyValueModificationsToEntity(
    "Health/Max",
    +this.template.Max,
    this.entity
  );
...
Health.prototype.RegisterHealthChanged = function(from) {
  this.CheckRegenTimer();
  this.UpdateActor();
  Engine.PostMessage(this.entity, MT_HealthChanged, {
    from: from,
    to: this.hitpoints
  });
};
