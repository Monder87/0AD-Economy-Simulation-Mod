...
Timer.prototype.Schema = "<a:component type='system'/><empty/>";
...
Timer.prototype.GetDay = function() {
  ...
  if (timeInSec % dayInSec == 0 && timeInSec != this.lastTimeSec) {
    this.day += 1;
    Engine.BroadcastMessage(MT_TimerDayChanged, { day: this.day });
    this.lastTimeSec = timeInSec;
  }
  return this.day;
};
...
Timer.prototype.OnUpdate = function(msg) {
  ...
  for (let [id, timer] of this.timers)
    if (timer.time <= this.time) run.push(id);
