...
var g_RangeTooltipString = {
  relative: {
    ...
    minRange: translate(
      "%(attackLabel)s %(damageTypes)s, %(rangeLabel)s %(minRange)s to %(maxRange)s (%(relativeRange)s) %(rangeUnit)s, %(rate)s"
    ),
    // Translation: For example: Ranged Attack: 12.0 Pierce, Range: 10 (+2) meters, Interval: 3 arrows / 2 seconds
    "no-minRange": translate(
      "%(attackLabel)s %(damageTypes)s, %(rangeLabel)s %(maxRange)s (%(relativeRange)s) %(rangeUnit)s, %(rate)s"
    )
  },
  "non-relative": {
    // Translation: For example: Ranged Attack: 12.0 Pierce, Range: 2 to 10 meters, Interval: 3 arrows / 2 seconds
    minRange: translate(
      "%(attackLabel)s %(damageTypes)s, %(rangeLabel)s %(minRange)s to %(maxRange)s %(rangeUnit)s, %(rate)s"
    ),
    // Translation: For example: Ranged Attack: 12.0 Pierce, Range: 10 meters, Interval: 3 arrows / 2 seconds
    "no-minRange": translate(
      "%(attackLabel)s %(damageTypes)s, %(rangeLabel)s %(maxRange)s %(rangeUnit)s, %(rate)s"
    )
  }
};
...
function getCostTypes() {
  return g_ResourceData
    .GetCodes()
    .concat(["population", "populationBonus", "time"]);
}
...
function getLocalizedResourceAmounts(resources) {
  let amounts = g_ResourceData
    .GetCodes()
    .filter(type => !!resources[type])
    .map(type =>
      sprintf(translate("%(amount)s %(resourceType)s"), {
        amount: resources[type],
        resourceType: resourceNameWithinSentence(type)
      })
    );

  if (amounts.length < 2) return amounts.join();
...
function getSecondsString(seconds) {
  return sprintf(
    translatePlural("%(time)s %(second)s", "%(time)s %(second)s", seconds),
    {
      time: seconds,
...
function getCurrentHealthTooltip(entState, label) {
  if (!entState.maxHitpoints) return "";
...
function attackRateDetails(template, type) {
  ...
  if (!template.buildingAI || type != "Ranged") return timeString;

  // Show either current rate from simulation or default count if the sim is not running
  let arrows =
    template.buildingAI.arrowCount || template.buildingAI.defaultArrowCount;
  let arrowString = sprintf(
    translatePlural(
      "%(arrowcount)s %(arrows)s",
      "%(arrowcount)s %(arrows)s",
      arrows
    ),
    {
...
function getArmorTooltip(template) {
  if (!template.armour) return "";

  return sprintf(translate("%(label)s %(details)s"), {
    label: headerFont(translate("Armor:")),
    details: Object.keys(template.armour)
      .map(dmgType =>
        sprintf(translate("%(damage)s %(damageType)s %(armorPercentage)s"), {
          damage: template.armour[dmgType].toFixed(1),
          damageType: unitFont(
            translateWithContext(
              "damage type",
              g_DamageTypes.GetNames()[dmgType]
            )
          ),
          armorPercentage:
            '[font="sans-10"]' +
            sprintf(translate("(%(armorPercentage)s)"), {
              armorPercentage: armorLevelToPercentageString(
                template.armour[dmgType]
              )
            }) +
            "[/font]"
        })
...
function damageTypesToText(dmg) {
  if (!dmg) return '[font="sans-12"]' + translate("(None)") + "[/font]";

  return g_DamageTypes
    .GetTypes()
    .filter(dmgType => dmg[dmgType])
    .map(dmgType =>
      sprintf(translate("%(damage)s %(damageType)s"), {
        damage: dmg[dmgType].toFixed(1),
        damageType: unitFont(
          translateWithContext("damage type", g_DamageTypes.GetNames()[dmgType])
        )
      })
    )
    .join(commaFont(translate(", ")));
...
function getAttackTooltip(template) {
  ...
  for (let type in template.attack) {
    if (type == "Slaughter") continue; // Slaughter is used to kill animals, so do not show it.
    ...
    if (type == "Capture" || type != "Ranged") {
      tooltips.push(
        sprintf(translate("%(attackLabel)s %(details)s, %(rate)s"), {
    ...
    tooltips.push(
      sprintf(
        g_RangeTooltipString[relativeRange ? "relative" : "non-relative"][
          minRange ? "minRange" : "no-minRange"
        ],
        {
          attackLabel: attackLabel,
          ...
          relativeRange:
            relativeRange > 0
              ? sprintf(translate("+%(number)s"), { number: relativeRange })
              : relativeRange,
          rangeUnit: unitFont(
            minRange || relativeRange
...
function getGarrisonTooltip(template) {
  if (!template.garrisonHolder) return "";
  ...
  if (template.garrisonHolder.buffHeal)
    tooltips.push(
      sprintf(
        translate("%(healRateLabel)s %(value)s %(health)s / %(second)s"),
        {
...
function getProjectilesTooltip(template) {
  if (!template.garrisonHolder || !template.buildingAI) return "";
    ...
    (entState.repairable.numBuilders
      ? sprintf(
          translatePlural(
            ...
            Math.round(
              entState.repairable.buildTime.timeRemaining -
                entState.repairable.buildTime.timeRemainingNew
            )
          ),
          {
            second: Math.round(
    ...
    (entState.foundation.numBuilders
      ? sprintf(
          translatePlural(
            ...
            Math.round(
              entState.foundation.buildTime.timeRemaining -
                entState.foundation.buildTime.timeRemainingNew
            )
          ),
          {
            second: Math.round(
 ...
 */
function getEntityCostComponentsTooltipString(
  template,
  entity,
  buildingsCountToTrainFullBatch = 1,
  fullBatchSize = 1,
  remainderBatch = 0
) {
  let totalCosts = multiplyEntityCosts(
    template,
    buildingsCountToTrainFullBatch * fullBatchSize + remainderBatch
  );
  if (template.cost.time)
    totalCosts.time = Math.ceil(
      template.cost.time *
        (entity
          ? Engine.GuiInterfaceCall("GetBatchTime", {
              entity: entity,
              batchSize:
                buildingsCountToTrainFullBatch > 0
                  ? fullBatchSize
                  : remainderBatch
            })
          : 1)
  ...
  for (let type of getCostTypes())
    if (type != "populationBonus" && totalCosts[type])
      costs.push(
        sprintf(translate("%(component)s %(cost)s"), {
...
function getGatherTooltip(template) {
  if (!template.resourceGatherRates) return "";
  ...
  for (let resource of g_ResourceData.GetResources()) {
    ...
    for (let subtype in resource.subtypes)
      if (subtype !== "ruins") types.push(resource.code + "." + subtype);

    let [rate, count] = types.reduce(
      (sum, t) => {
    ...
    if (rate > 0) rates[resource.code] = +(rate / count).toFixed(1);
  }
  ...
  return sprintf(translate("%(label)s %(details)s"), {
    ...
    details: Object.keys(rates)
      .map(type =>
        sprintf(translate("%(resourceIcon)s %(rate)s"), {
...
function getResourceTrickleTooltip(template) {
  if (!template.resourceTrickle) return "";

  let resCodes = g_ResourceData
    .GetCodes()
    .filter(res => !!template.resourceTrickle.rates[res]);
  if (!resCodes.length) return "";
  ...
  if (sameTypes)
    ...
    for (let resource in resourceCount)
      out.push(
        sprintf(
          translate(
            "%(resourceIcon)s %(minimum)s to %(resourceIcon)s %(maximum)s"
          ),
          {
...
function getEntityCostTooltip(
  ...
  if (template.wallSet) {
    ...
    let wallCosts = getWallPieceTooltip([
      templateShort,
      templateMedium,
      templateLong
    ]);
    ...
    return (
      sprintf(translate("Walls:  %(costs)s"), { costs: wallCosts.join("  ") }) +
      "\n" +
  ...
  if (template.cost) {
    let costs = getEntityCostComponentsTooltipString(
      template,
      entity,
      buildingsCountToTrainFullBatch,
      fullBatchSize,
    ...
    if (costs)
      ...
      return sprintf(translate("%(label)s %(costs)s"), {
        label: headerFont(translate("Raw Material:")),
        costs: costs
      });
  }
...
function getPrice(price) {
  return sprintf(translate("%(label)s %(price)s"), {
    label:
      headerFont(translate("Price:")) +
      '[font="sans-12"]' +
      '[icon="icon_money"]' +
      "[/font]",
    price: price
...
function getAurasTooltip(template) {
  let auras =
    template.auras ||
    (template.wallSet &&
      GetTemplateData(template.wallSet.templates.long).auras);
  if (!auras) return "";

  let tooltips = [];
  for (let auraID in auras) {
    let tooltip = sprintf(translate("%(auralabel)s %(aurainfo)s"), {
      auralabel: headerFont(
        sprintf(translate("%(auraname)s:"), {
...
function getEntityNames(template) {
  if (!template.name.specific) return template.name.generic;
...
function getEntityNamesFormatted(template) {
  if (!template.name.specific)
    return setStringTags(
      ...
      g_TooltipTextFormats.nameSpecificBig
    );

  // Translation: Example: "Epibátēs Athēnaîos [font="sans-bold-16"](Athenian Marine)[/font]"
  return sprintf(
    translate("%(specificName)s %(fontStart)s(%(genericName)s)%(fontEnd)s"),
    {
      specificName:
        setStringTags(
          template.name.specific[0],
          g_TooltipTextFormats.nameSpecificBig
        ) +
        setStringTags(
          template.name.specific.slice(1).toUpperCase(),
          g_TooltipTextFormats.nameSpecificSmall
        ),
      genericName: template.name.generic,
...
function getVisibleEntityClassesFormatted(template) {
  if (
    !template.visibleIdentityClasses ||
    !template.visibleIdentityClasses.length
  )
    return "";

  return (
    headerFont(translate("Classes:")) +
    " " +
    bodyFont(
      template.visibleIdentityClasses
        .map(c => translate(c))
...
function getLootTooltip(template) {
  if (!template.loot && !template.resourceCarrying) return "";
  ...
  for (let type of g_ResourceData.GetCodes().concat(["xp"])) {
    ...
    lootLabels.push(
      sprintf(translate("%(component)s %(loot)s"), {
        component: resourceIcon(type),
        loot: loot
      })
    );
