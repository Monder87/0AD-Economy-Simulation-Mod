...
function initMenu() {
  Engine.GetGUIObjectByName("menu").size =
    "100%-164 " + MENU_TOP + " 100% " + MENU_BOTTOM;
...
function lobbyDialogButton() {
  if (!Engine.HasXmppClient()) return;
...
function optionsPageClosed(data) {
  for (let callback of data) if (global[callback]) global[callback]();

  resumeGame();
}

function openChat(command = "") {
  if (g_Disconnected) return;
...
function resizeDiplomacyDialog() {
  ...
  let tribSize = Engine.GetGUIObjectByName("diplomacyPlayer[0]_tribute[0]")
    .size;
  let widthOffset =
    g_ResourceData.GetCodes().length * (tribSize.right - tribSize.left) / 2;
  size.left -= widthOffset;
  size.right += widthOffset;

  let firstRow = Engine.GetGUIObjectByName("diplomacyPlayer[0]").size;
  let heightOffset =
    (g_Players.length - 1) * (firstRow.bottom - firstRow.top) / 2;
...
function resizeChatWindow() {
  ...
  if (extended) {
    chatDialogPanel.size = Engine.GetGUIObjectByName(
      "chatDialogPanelLarge"
    ).size;
    // Adjust the width so that the chat history is in the golden ratio
    let chatHistory = Engine.GetGUIObjectByName("chatHistory");
    let height =
      chatHistory.getComputedSize().bottom - chatHistory.getComputedSize().top;
...
function updateChatHistory() {
  ...
  Engine.GetGUIObjectByName("chatHistory").caption = g_ChatHistory
    .filter(msg => msg.filter[selected])
    .map(
      msg =>
...
function onToggleChatWindowExtended() {
  saveSettingAndWriteToUserConfig(
    ...
    String(Engine.GetGUIObjectByName("extendedChat").checked)
  );
...
function toggleDiplomacy() {
  ...
  if (!open) openDiplomacy();
}

function updateDiplomacy(opening = false) {
  if (g_ViewedPlayer < 1 || !g_IsDiplomacyOpen) return;
  ...
  for (let i = 1; i < g_Players.length; ++i) {
    ...
    let playerInactive =
      isPlayerObserver(g_ViewedPlayer) || isPlayerObserver(i);
    let hasAllies =
      g_Players.filter(player => player.isMutualAlly[g_ViewedPlayer]).length >
      1;
    ...
    diplomacyFormatStanceButtons(
      ...
      myself ||
        playerInactive ||
        isCeasefireActive ||
        g_Players[g_ViewedPlayer].teamsLocked
    );
    // Tribute buttons do not need to be updated onTick, and should not because of massTributing
    if (opening) diplomacyFormatTributeButtons(i, myself || playerInactive);
    diplomacyFormatAttackRequestButton(
      ...
      myself ||
        ...
        isCeasefireActive ||
        !hasAllies ||
        !g_Players[i].isEnemy[g_ViewedPlayer]
    );
    diplomacyFormatSpyRequestButton(
      i,
      myself ||
        playerInactive ||
        (g_Players[i].isMutualAlly[g_ViewedPlayer] && hasSharedLos)
    );
  }

  let diplomacyCeasefireCounter = Engine.GetGUIObjectByName(
    "diplomacyCeasefireCounter"
  );
...
function diplomacySetupTexts(i, rowsize) {
  ...
  let diplomacyPlayerName = Engine.GetGUIObjectByName(
    "diplomacyPlayerName[" + (i - 1) + "]"
  );
  diplomacyPlayerName.caption = colorizePlayernameByID(i);
  diplomacyPlayerName.tooltip = translateAISettings(
    g_GameAttributes.settings.PlayerData[i]
  );

  Engine.GetGUIObjectByName("diplomacyPlayerCiv[" + (i - 1) + "]").caption =
    g_CivData[g_Players[i].civ].Name;
  ...
  Engine.GetGUIObjectByName("diplomacyPlayerTheirs[" + (i - 1) + "]").caption =
    i == g_ViewedPlayer
      ...
      : g_Players[i].isAlly[g_ViewedPlayer]
        ? translate("Ally")
        : g_Players[i].isNeutral[g_ViewedPlayer]
          ? translate("Neutral")
          : translate("Enemy");
}
...
function diplomacyFormatStanceButtons(i, hidden) {
  for (let stance of ["Ally", "Neutral", "Enemy"]) {
    ...
    button.onPress = (function(player, stance) {
      return function() {
        Engine.PostNetworkCommand({
          type: "diplomacy",
          player: i,
...
function diplomacyFormatTributeButtons(i, hidden) {
  ...
  for (let resCode of resCodes) {
    let button = Engine.GetGUIObjectByName(
      "diplomacyPlayer[" + (i - 1) + "]_tribute[" + r + "]"
    );
    if (!button) {
      warn(
    ...
    Engine.GetGUIObjectByName(
      "diplomacyPlayer[" + (i - 1) + "]_tribute[" + r + "]_image"
    ).sprite =
      "stretched:session/icons/resources/" + resCode + ".png";
    button.hidden = hidden;
    ...
    button.onPress = (function(i, resCode, button) {
      ...
      return function() {
        ...
        let amounts = {};
        for (let res of resCodes) amounts[res] = 0;
        ...
        if (!isBatchTrainPressed) g_FlushTributing();
      };
    })(i, resCode, button);
  }
}
...
function diplomacyFormatSpyRequestButton(i, hidden) {
  let button = Engine.GetGUIObjectByName(
    "diplomacySpyRequest[" + (i - 1) + "]"
  );
  let template = GetTemplateData("special/spy");
  button.hidden =
    hidden ||
    !template ||
    GetSimState().players[g_ViewedPlayer].disabledTemplates["special/spy"];
  ...
  button.enabled =
    ...
    !(
      ...
      g_BribeButtonsWaiting[g_ViewedPlayer].indexOf(i) != -1
    );
  let modifier = "";
  let tooltips = [
    translate(
  ...
  if (!button.enabled) modifier = "color:0 0 0 127:grayscale:";
  else {
    if (template.requiredTechnology) {
      let technologyEnabled = Engine.GuiInterfaceCall(
    ...
    if (template.cost) {
      ...
      for (let res in template.cost)
        modifiedTemplate.cost[res] = Math.floor(
          GetSimState().players[i].spyCostMultiplier * template.cost[res]
        );
      ...
      let costRatio = Engine.GetTemplate("special/spy").VisionSharing
        .FailureCostRatio;
      if (costRatio > 0) {
        tooltips.push(translate("A failed bribe will cost you:"));
        for (let res in modifiedTemplate.cost)
          modifiedTemplate.cost[res] = Math.floor(
            costRatio * modifiedTemplate.cost[res]
          );
        tooltips.push(getEntityCostTooltip(modifiedTemplate));
      }
...
function resizeTradeDialog() {
  ...
  width +=
    g_ResourceData.GetCodes().length * (tradeSize.right - tradeSize.left);
...
function openTrade() {
  ...
  let updateTradeButtons = function() {
    for (let res in button) {
      ...
      button[res].sel.hidden =
        !controlsPlayer(g_ViewedPlayer) || res != currTradeSelection;
      button[res].up.hidden =
        !controlsPlayer(g_ViewedPlayer) ||
        res == currTradeSelection ||
        proba[res] == 100 ||
        proba[currTradeSelection] == 0;
      button[res].dn.hidden =
        !controlsPlayer(g_ViewedPlayer) ||
        res == currTradeSelection ||
        proba[res] == 0 ||
        proba[currTradeSelection] == 100;
    }
  };
  ...
  for (let i = 0; i < resCodes.length; ++i) {
    ...
    if (!barterResource) {
      warn(
        "Current GUI limits prevent displaying more than " +
          i +
          " resources in the barter dialog!"
      );
      break;
    }
...
function updateTraderTexts() {
  ...
  Engine.GetGUIObjectByName("traderCountText").caption =
...
function barterOpenCommon(resourceCode, idx, prefix) {
  ...
  for (let action of g_BarterActions)
    barterButton[action] = Engine.GetGUIObjectByName(
      prefix + action + "Button[" + idx + "]"
    );

  let resource = resourceNameWithinSentence(resourceCode);
  barterButton.Buy.tooltip = sprintf(translate("Buy %(resource)s"), {
    resource: resource
  });
  barterButton.Sell.tooltip = sprintf(translate("Sell %(resource)s"), {
    resource: resource
  });
...
function barterUpdateCommon(resourceCode, idx, prefix, player) {
  ...
  for (let action of g_BarterActions) {
    ...
    barterIcon[action] = Engine.GetGUIObjectByName(
      prefix + action + "Icon[" + idx + "]"
    );
    barterAmount[action] = Engine.GetGUIObjectByName(
      prefix + action + "Amount[" + idx + "]"
    );
  }
  let selectionIcon = Engine.GetGUIObjectByName(
    prefix + "SellSelection[" + idx + "]"
  );
  ...
  barterAmount.Buy.caption =
    "+" +
    Math.round(
      prices.sell[g_BarterSell] / prices.buy[resourceCode] * amountToSell
    );
...
function updateBarterButtons() {
  ...
  if (canBarter)
    g_ResourceData.GetCodes().forEach((resCode, i) => {
...
function getIdleLandTradersText(traderNumber) {
  ...
  let messageTypes = {
    active: {
      garrisoned: {
        "no-inactive": translate(
          "%(openingTradingString)s, and %(garrisonedString)s."
        ),
        inactive: translate(
    ...
    "no-active": {
      garrisoned: {
        ...
        inactive: translate(
          "%(openingGarrisonedString)s, and %(inactiveString)s."
        )
      },
      "no-garrisoned": {
        inactive: translatePlural(
          "There is %(inactiveString)s.",
          "There are %(inactiveString)s.",
          inactive
        ),
  ...
  let message =
    messageTypes[active ? "active" : "no-active"][
      garrisoned ? "garrisoned" : "no-garrisoned"
    ][inactive ? "inactive" : "no-inactive"];
...
function closeEconomy() {
  g_IsEconomyOpen = false;
  Engine.GetGUIObjectByName("tradeDialogPanel").hidden = true;
}

function toggleEconomy() {
  let open = g_IsEconomyOpen;
  closeOpenDialogs();
...
function toggleTutorial() {
  ...
  tutorialPanel.hidden =
    !tutorialPanel.hidden || !Engine.GetGUIObjectByName("tutorialText").caption;
}
...
function updateGameSpeedControl() {
  ...
  if (gameSpeedIdx == -1) warn("Unknown gamespeed:" + simRate);
...
function openObjectives() {
  ...
  let objectivesPlayerstate = Engine.GetGUIObjectByName(
    "objectivesPlayerstate"
  );
  ...
  gameDescriptionSize.top = Engine.GetGUIObjectByName(
    isActive ? "objectivesTitle" : "objectivesPlayerstate"
  ).size.bottom;
...
function openGameSummary() {
  ...
  Engine.PushGuiPage("page_summary.xml", {
    sim: {
      ...
      playerStates: extendedSimState.players.filter(
        (state, player) =>
          g_IsObserver ||
          player == 0 ||
          player == g_ViewedPlayer ||
          (extendedSimState.players[g_ViewedPlayer].hasSharedLos &&
...
function toggleDeveloperOverlay() {
  if (!g_GameAttributes.settings.CheatsEnabled && !g_IsReplay) return;
  ...
  if (Engine.GetPlayerID() == -1) submitChatDirectly(message);
  else
    Engine.PostNetworkCommand({
      type: "aichat",
      message: message,
...
function formatTributeTooltip(playerID, resourceCode, amount) {
  return sprintf(
    translate(
      "Tribute %(resourceAmount)s %(resourceType)s to %(playerName)s. Shift-click to tribute %(greaterAmount)s."
    ),
    {
