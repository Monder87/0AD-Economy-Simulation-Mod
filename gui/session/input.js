...
function updateCursorAndTooltip() {
  ...
  if (!cursorSet) Engine.ResetCursor();

  if (!tooltipSet) informationTooltip.hidden = true;

  var placementTooltip = Engine.GetGUIObjectByName("placementTooltip");
  if (placementSupport.tooltipMessage)
    placementTooltip.sprite = placementSupport.tooltipError
      ? "BackgroundErrorTooltip"
      : "BackgroundInformationTooltip";
  ...
  if (placementSupport.mode === "building") {
    if (placementSupport.template && placementSupport.position) {
          ...
          if (result.translateMessage)
            if (result.pluralMessage)
              message = translatePlural(
                result.message,
                result.pluralMessage,
                result.pluralCount
              );
            else message = translate(message);
      ...
      if (placementSupport.attack && placementSupport.attack.Ranged) {
        ...
        var averageRange = Math.round(
          Engine.GuiInterfaceCall("GetAverageRangeForBuildings", cmd) -
            cmd.range
        );
        ...
        placementSupport.tooltipMessage =
          sprintf(
            translatePlural(
              "Basic range: %(range)s meter",
              "Basic range: %(range)s meters",
              range
            ),
            { range: range }
          ) +
          ...
          sprintf(
            translatePlural(
              "Average bonus range: %(range)s meter",
              "Average bonus range: %(range)s meters",
              averageRange
            ),
            { range: averageRange }
          );
...
function determineAction(x, y, fromMinimap) {
  ...
  if (!g_DevSettings.controlAll && !allOwnedByPlayer) return undefined;

  var target = undefined;
  if (!fromMinimap) {
    var ent = Engine.PickEntityAtPoint(x, y);
    if (ent != INVALID_ENTITY) target = ent;
  }
  ...
  actions.sort(
    (a, b) => g_UnitActions[a].specificness - g_UnitActions[b].specificness
  );
...
function tryPlaceBuilding(queued) {
  if (placementSupport.mode !== "building") {
    error(
      "tryPlaceBuilding expected 'building', got '" +
        placementSupport.mode +
        "'"
    );
    return false;
  }
  ...
  if (!queued) placementSupport.Reset();
...
function tryPlaceWall(queued) {
  ...
  if (!(wallPlacementInfo === false || typeof wallPlacementInfo === "object")) {
    error(
      "Invalid updateBuildingPlacementPreview return value: " +
        uneval(wallPlacementInfo)
    );
    return false;
  }

  if (!wallPlacementInfo) return false;
...
var unitFilters = {
  ...
  isMilitary: entity => {
    var entState = GetEntityState(entity);
    return entState && g_MilitaryTypes.some(c => hasClass(entState, c));
  },
  ...
  isWounded: entity => {
    ...
    return (
      ...
      100 * entState.hitpoints <=
        entState.maxHitpoints *
          Engine.ConfigDB_GetValue(
            "user",
            "gui.session.woundedunithotkeythreshold"
          )
    );
  },
  isAnything: entity => {
...
function getPreferredEntities(ents) {
  ...
  var filters = [
    unitFilters.isUnit,
    unitFilters.isDefensive,
    unitFilters.isAnything
  ];
...
function handleInputBeforeGui(ev, hoveredObject) {
  ...
  if (
    !mouseIsOverObject &&
    (ev.type == "mousebuttonup" || ev.type == "mousebuttondown") &&
    ...
    case INPUT_BANDBOXING:
      ...
      switch (ev.type) {
        case "mousemotion":
          ...
          var ents = Engine.PickPlayerEntitiesInRect(
            rect[0],
            rect[1],
            rect[2],
            rect[3],
        ...
        case "mousebuttonup":
          if (ev.button == SDL_BUTTON_LEFT) {
            ...
            var ents = getPreferredEntities(
              Engine.PickPlayerEntitiesInRect(
                rect[0],
                rect[1],
                rect[2],
                rect[3],
        ...
        case "mousebuttonup":
          if (ev.button == SDL_BUTTON_LEFT) {
            ...
            if (tryPlaceBuilding(queued)) {
              if (queued) inputState = INPUT_BUILDING_PLACEMENT;
              else inputState = INPUT_NORMAL;
            } else {
      ...
      switch (ev.type) {
        case "mousemotion":
          ...
          if (result && result.cost) {
            var neededResources = Engine.GuiInterfaceCall(
              "GetNeededResources",
              { cost: result.cost }
            );
            placementSupport.tooltipMessage = [
        ...
        case "mousebuttondown":
          if (ev.button == SDL_BUTTON_LEFT) {
            ...
            if (tryPlaceWall(queued)) {
              ...
              placementSupport.tooltipMessage = translate(
                "Cannot build wall here!"
              );
      ...
      switch (ev.type) {
        case "mousemotion":
          ...
          if (g_DragStart.distanceTo(ev) >= maxDragDelta) {
            ...
            placementSupport.angle = placementSupport.position.horizAngleTo(
              Engine.GetTerrainAtScreenPoint(ev.x, ev.y)
            );
          } else {
...
function handleInputAfterGui(ev) {
  ...
  if (ev.hotkey === undefined) ev.hotkey = null;
  ...
  if (!g_IsNetworked && Engine.GetGUIObjectByName("devTimeWarp").checked) {
    if (ev.type == "hotkeydown" && ev.hotkey == "session.timewarp.fastforward")
      ...
      ev.type == "hotkeyup" &&
      ev.hotkey == "session.timewarp.fastforward"
    )
    ...
    case INPUT_NORMAL:
      switch (ev.type) {
        case "mousemotion":
          ...
          if (ent != INVALID_ENTITY) g_Selection.setHighlightList([ent]);
          else g_Selection.setHighlightList([]);
        ...
        case "hotkeydown":
          if (ev.hotkey.indexOf("selection.group.") == 0) {
            let now = Date.now();
            if (
              now - doublePressTimer < doublePressTime &&
              ev.hotkey == prevHotkey
            ) {
          ...
          if (ev.button == SDL_BUTTON_LEFT) {
            ...
            if (
              ...
              if (
                ...
                !Engine.HotkeyIsPressed("selection.remove")
              ) {
                g_Selection.reset();
                resetIdleUnit();
              }
            ...
            var ents = [];
            if (ev.clicks == 1) ents = [clickedEntity];
            else {
              ...
              if (ev.clicks == 2) {
                ...
                templateToMatch = GetEntityState(clickedEntity).identity
                  .selectionGroupName;
                if (templateToMatch) matchRank = false;
                else
              ...
              ents = Engine.PickSimilarPlayerEntities(
                templateToMatch,
                showOffscreen,
                matchRank,
                false
    ...
    case INPUT_UNIT_POSITION_START:
      switch (ev.type) {
        case "mousemotion":
          ...
          if (
            g_DragStart.distanceToSquared(ev) >= Math.square(g_MaxDragDelta)
          ) {
        ...
        case "mousebuttonup":
          ...
          if (ev.button == SDL_BUTTON_RIGHT) {
            ...
            if (action) return doAction(action, ev);
          }
          break;
      }
      break;
        ...
        case "mousemotion":
          ...
          if (placementSupport.mode === "wall") {
            ...
            if (
              ...
              Engine.GuiInterfaceCall("GetNeededResources", {
...
function doAction(action, ev) {
  if (!controlsPlayer(g_ViewedPlayer)) return false;
  ...
  if (g_UnitActions[action.type] && g_UnitActions[action.type].execute) {
    ...
    if (orderone) {
      ...
      let unit = selection.find(entity =>
        ["preSelectedActionCheck", "hotkeyActionCheck", "actionCheck"].some(
          method =>
            g_UnitActions[action.type][method] &&
            g_UnitActions[action.type][method](action.target || undefined, [
              entity
            ])
        )
      );
...
function positionUnitsFreehandSelectionMouseMove(ev) {
  ...
  if (
    !g_FreehandSelection_InputLine.length ||
    target.distanceToSquared(
      g_FreehandSelection_InputLine[g_FreehandSelection_InputLine.length - 1]
...
function positionUnitsFreehandSelectionMouseUp(ev) {
  ...
  if (!ev.button == SDL_BUTTON_RIGHT) return true;

  let lengthOfLine = 0;
  for (let i = 1; i < inputLine.length; ++i)
    lengthOfLine += inputLine[i].distanceTo(inputLine[i - 1]);

  let selection = g_Selection
    .toList()
    .filter(ent => !!GetEntityState(ent).unitAI)
    .sort((a, b) => a - b);

  // Checking the line for a minimum length to save performance.
  if (
    lengthOfLine < g_FreehandSelection_MinLengthOfLine ||
    selection.length < g_FreehandSelection_MinNumberOfUnits
  ) {
    let action = determineAction(ev.x, ev.y);
  ...
  entityDistribution = entityDistribution.concat(
    new Array(selection.length - entityDistribution.length).fill(
      inputLine[inputLine.length - 1]
    )
  );

  if (
    Vector2D.from3D(GetEntityState(selection[0]).position).distanceTo(
      entityDistribution[0]
    ) +
      Vector2D.from3D(
        GetEntityState(selection[selection.length - 1]).position
      ).distanceTo(entityDistribution[selection.length - 1]) >
    Vector2D.from3D(GetEntityState(selection[0]).position).distanceTo(
      entityDistribution[selection.length - 1]
    ) +
      Vector2D.from3D(
        GetEntityState(selection[selection.length - 1]).position
      ).distanceTo(entityDistribution[0])
  )
    entityDistribution.reverse();

  Engine.PostNetworkCommand({
    type: Engine.HotkeyIsPressed("session.attackmove")
      ? "attack-walk-custom"
      : "walk-custom",
    entities: selection,
    targetPositions: entityDistribution.map(pos => pos.toFixed(2)),
    targetClasses: Engine.HotkeyIsPressed("session.attackmoveUnit")
      ? { attack: ["Unit"] }
...
function handleMinimapEvent(target) {
  ...
  if (inputState != INPUT_NORMAL) return false;
...
function getEntityLimitAndCount(playerState, entType) {
  ...
  if (!playerState.entityLimits) return ret;
  let template = GetTemplateData(entType);
  let entCategory =
    (template.trainingRestrictions && template.trainingRestrictions.category) ||
...
function OnTrainMouseWheel(dir) {
  if (Engine.HotkeyIsPressed("session.batchtrain"))
    g_BatchSize +=
      dir / Engine.ConfigDB_GetValue("user", "gui.session.scrollbatchratio");
  if (g_BatchSize < 1 || !Number.isFinite(g_BatchSize)) g_BatchSize = 1;
}
...
function getBuildingsWhichCanTrainEntity(entitiesToCheck, trainEntType) {
  return entitiesToCheck.filter(entity => {
    ...
    return (
      state &&
      state.production &&
      state.production.entities.length &&
...
function addTrainingByPosition(position) {
  ...
  if (!playerState || !selection.length) return;
...
function addTrainingToQueue(selection, trainEntType, playerState) {
  let appropriateBuildings = getBuildingsWhichCanTrainEntity(
    selection,
    trainEntType
  );

  let canBeAddedCount = getEntityLimitAndCount(playerState, trainEntType)
    .canBeAddedCount;

  let decrement = Engine.HotkeyIsPressed("selection.remove");
  let template;
  if (!decrement) template = GetTemplateData(trainEntType);

  // Batch training only possible if we can train at least 2 units
  if (
    Engine.HotkeyIsPressed("session.batchtrain") &&
    (canBeAddedCount == undefined || canBeAddedCount > 1)
  ) {
    if (inputState == INPUT_BATCHTRAINING) {
      ...
      if (
        g_BatchTrainingEntities.length == selection.length &&
        g_BatchTrainingEntities.every((ent, i) => ent == selection[i]) &&
        g_BatchTrainingType == trainEntType
      ) {
        if (decrement) {
          --g_NumberOfBatches;
          if (g_NumberOfBatches <= 0) inputState = INPUT_NORMAL;
        } else if (
          canBeAddedCount == undefined ||
          canBeAddedCount >
            g_NumberOfBatches *
              getBatchTrainingSize() *
              appropriateBuildings.length
        ) {
          if (
            Engine.GuiInterfaceCall("GetNeededResources", {
              cost: multiplyEntityCosts(
                template,
                (g_NumberOfBatches + 1) * getBatchTrainingSize()
              )
            })
          )
            return;
...
function getTrainingStatus(selection, trainEntType, playerState) {
  ...
  if (
    ...
    canBeAddedCount = getEntityLimitAndCount(playerState, trainEntType)
      .canBeAddedCount;
  ...
  if (
    ...
    buildingsCountToTrainFullBatch = Math.floor(
      canBeAddedCount / nextBatchTrainingCount
    );
...
function flushTrainingBatch() {
  ...
  if (
    g_BatchTrainingEntityAllowedCount !== undefined &&
    g_BatchTrainingEntityAllowedCount <
      batchedSize * appropriateBuildings.length
  ) {
    // Train as many full batches as we can
    let buildingsCountToTrainFullBatch = Math.floor(
      g_BatchTrainingEntityAllowedCount / batchedSize
    );
    Engine.PostNetworkCommand({
      type: "train",
...
function performGroup(action, groupId) {
  switch (action) {
    ...
    case "add":
      var toSelect = [];
      g_Groups.update();
      for (var ent in g_Groups.groups[groupId].ents) toSelect.push(+ent);

      if (action != "add") g_Selection.reset();
    ...
    case "breakUp":
      ...
      if (action == "save") g_Groups.addEntities(groupId, g_Selection.toList());
...
function findIdleUnit(classes) {
  ...
  if (
    selectall ||
    classes.length != lastIdleClasses.length ||
    !classes.every((v, i) => v === lastIdleClasses[i])
  )
    resetIdleUnit();
  ...
  var data = {
    ...
    idleClasses: classes
      .slice(currIdleClassIndex, classes.length)
      .concat(classes.slice(0, currIdleClassIndex))
  };
  if (!selectall) {
    data.limit = 1;
  ...
  if (position) Engine.CameraMoveTo(position.x, position.z);
  // Move the idle class index to the first class an idle unit was found for.
  var indexChange = data.idleClasses.findIndex(elem =>
    MatchesClassList(entityState.identity.classes, elem)
  );
