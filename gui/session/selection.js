...
function _setHighlight(ents, alpha, selected) {
  if (ents.length)
    Engine.GuiInterfaceCall("SetSelectionHighlight", {
      entities: ents,
      alpha: alpha,
      selected: selected
    });
...
EntityGroups.prototype.add = function(ents) {
  for (let ent of ents) {
    if (!this.ents[ent]) {
      ...
      var key =
        GetTemplateData(templateName).selectionGroupName || templateName;
      ...
      if (this.groups[key]) this.groups[key] += 1;
      else this.groups[key] = 1;
...
EntityGroups.prototype.removeEnt = function(ent) {
  ...
  if (this.groups[templateName] == 0) delete this.groups[templateName];
};
...
EntityGroups.prototype.rebuildGroup = function(renamed) {
  ...
  var toAdd = [];
  for (var ent in oldGroup) toAdd.push(renamed[ent] ? renamed[ent] : +ent);
...
EntityGroups.prototype.getTotalCount = function() {
  let totalCount = 0;
  for (let templateName in this.groups) totalCount += this.groups[templateName];
  return totalCount;
};
...
EntityGroups.prototype.getEntsByName = function(templateName) {
  ...
  for (var ent in this.ents)
    if (this.ents[ent] == templateName) ents.push(+ent);
 ...
 */
EntitySelection.prototype.makePrimarySelection = function(
  templateName,
  modifierKey
) {
  ...
  var ents = [];
  if (modifierKey) ents = this.groups.getEntsByNameInverse(key);
  else ents = this.groups.getEntsByName(key);
...
EntitySelection.prototype.getTemplateNames = function() {
  ...
  for (let ent in this.selected) {
    ...
    if (entState) templateNames.push(entState.template);
  }
...
EntitySelection.prototype.update = function() {
  ...
  let changed = false;
  let removeOwnerChanges =
    !g_IsObserver && !g_DevSettings.controlAll && this.toList().length > 1;
  ...
  for (let ent in this.selected) {
    ...
    if (
      (entState.visibility == "hidden" && !entState.mirage) ||
...
EntitySelection.prototype.addList = function(ents, quiet, force = false) {
  ...
  if (firstEntState && firstEntState.player != g_ViewedPlayer && !force) return;

  let i = 1;
  let added = [];

  for (let ent of ents) {
    if (selection.length + i > g_MaxSelectionSize) break;

    if (this.selected[ent]) continue;

    var entState = GetEntityState(ent);
    if (!entState) continue;

    let isUnowned =
      (g_ViewedPlayer != -1 && entState.player != g_ViewedPlayer) ||
      (g_ViewedPlayer == -1 && entState.player == 0);

    // Don't add unowned entities to the list, unless a single entity was selected
    if (isUnowned && (ents.length > 1 || selection.length) && !force) continue;
...
EntitySelection.prototype.setHighlightList = function(ents) {
  ...
  for (let ent in this.highlighted)
    if (!highlighted[ent] && !this.selected[ent]) removed.push(+ent);

  // Add new highlighting for units that aren't already highlighted
  for (let ent of ents)
    if (!this.highlighted[ent] && !this.selected[ent]) added.push(+ent);
...
EntitySelection.prototype.onChange = function() {
  this.dirty = true;
  if (this.isSelection) onSelectionChange();
};
...
// === Economy Var == \\
var g_allConsumingProducts = undefined;
...
function onSelectionChange() {
  ...
  // === Economy Var == \\
  g_allConsumingProducts = undefined;
}
...
EntityGroupsContainer.prototype.addEntities = function(groupName, ents) {
  for (let ent of ents)
    for (let group of this.groups) if (ent in group.ents) group.removeEnt(ent);
...
EntityGroupsContainer.prototype.update = function() {
  ...
  for (let group of this.groups)
    for (var ent in group.ents) {
      var entState = GetEntityState(+ent);
      // Remove deleted units
      if (!entState) group.removeEnt(ent);
    }
};
