...
const g_VictoryDurations = prepareForDropdown(
  g_Settings && g_Settings.VictoryDurations
);
...
function GetTechnologyData(technologyName, civ) {
  if (!g_TechnologyData[civ]) g_TechnologyData[civ] = {};

  if (!(technologyName in g_TechnologyData[civ])) {
    let template = GetTechnologyDataHelper(
      TechnologyTemplates.Get(technologyName),
      civ,
      g_ResourceData
    );
    translateObjectKeys(template, [
      "specific",
      "generic",
      "description",
      "tooltip",
...
function init(initData, hotloadData) {
  ...
  g_PlayerAssignments = initData
    ? initData.playerAssignments
    : { local: { player: 1 } };
  ...
  if (initData) {
    ...
    if (initData.savedGUIData) restoreSavedGameData(initData.savedGUIData);
  }
  ...
  if (hotloadData) g_Selection.selected = hotloadData.selection;
...
function updateDiplomacyColorsButton() {
  ...
  let diplomacyColorsButton = Engine.GetGUIObjectByName(
    "diplomacyColorsButton"
  );
  ...
  Engine.GetGUIObjectByName(
    "diplomacyColorsWindowButtonIcon"
  ).sprite = g_DiplomacyColorsToggle
    ...
    let getDiplomacyColor = stance =>
      guiToRgbColor(
        Engine.ConfigDB_GetValue(
          "user",
          "gui.session.diplomacycolors." + stance
        )
      ) ||
      guiToRgbColor(
        Engine.ConfigDB_GetValue(
    ...
    for (let i = 1; i < g_Players.length; ++i)
      if (g_ViewedPlayer <= 0) {
        ...
        g_DisplayedPlayerColors[i] =
          g_Players[teamRepresentatives[team] || i].color;
        ...
        g_DisplayedPlayerColors[i] =
          g_ViewedPlayer == i
            ? getDiplomacyColor("self")
            : g_Players[g_ViewedPlayer].isAlly[i]
              ? getDiplomacyColor("ally")
              : g_Players[g_ViewedPlayer].isNeutral[i]
                ? getDiplomacyColor("neutral")
                : getDiplomacyColor("enemy");

    g_DisplayedPlayerColors[0] = g_Players[0].color;
  } else g_DisplayedPlayerColors = g_Players.map(player => player.color);
...
function updateHotkeyTooltips() {
  ...
  Engine.GetGUIObjectByName("barterHelp").tooltip = sprintf(
    translate(
      "Start by selecting the resource you wish to sell from the upper row. For each time the lower buttons are pressed, %(quantity)s of the upper resource will be sold for the displayed quantity of the lower. Press and hold %(hotkey)s to temporarily multiply the traded amount by %(multiplier)s."
    ),
    {
...
function initPanelEntities() {
  Engine.GetGUIObjectByName("panelEntityPanel").children.forEach(
    (button, slot) => {
      button.onPress = function() {
        let panelEnt = g_PanelEntities.find(
          ent => ent.slot !== undefined && ent.slot == slot
        );
        if (!panelEnt) return;

        if (!Engine.HotkeyIsPressed("selection.add")) g_Selection.reset();
      ...
      button.onDoublePress = function() {
        ...
        if (panelEnt) selectAndMoveTo(getEntityOrHolder(panelEnt.ent));
      };
    }
  );
}
...
function initViewedPlayerDropdown() {
  ...
  Engine.GetGUIObjectByName("viewPlayer").selected =
    player && player.state == "defeated" ? 0 : Engine.GetPlayerID() + 1;
}
...
function updateViewedPlayerDropdown() {
  ...
  viewPlayer.list = [translate("Observer")].concat(
    g_Players.map(
...
function selectViewPlayer(playerID) {
  if (playerID < -1 || playerID > g_Players.length - 1) return;

  if (g_ShowAllStatusBars) recalculateStatusBarDisplay(true);

  g_IsObserver = isPlayerObserver(Engine.GetPlayerID());

  if (g_IsObserver || g_DevSettings.changePerspective) {
    if (g_ViewedPlayer != playerID) clearSelection();
  ...
  if (g_IsDiplomacyOpen) openDiplomacy();

  if (g_IsTradeOpen) openTrade();
}
...
function controlsPlayer(playerID) {
  ...
  return (
    (playerStates[Engine.GetPlayerID()] &&
...
function playersFinished(players, victoryString, won) {
  ...
  if (players.indexOf(Engine.GetPlayerID()) != -1) reportGame();
  ...
  Engine.GetGUIObjectByName("viewPlayer").selected = won
    ? g_ViewedPlayer + 1
    : 0;
  ...
  global.music.setState(
    won ? global.music.states.VICTORY : global.music.states.DEFEAT
  );
...
function resignGame(leaveGameAfterResign) {
  if (g_IsObserver || g_Disconnected) return;

  Engine.PostNetworkCommand({
    type: "resign"
  });

  if (!leaveGameAfterResign) resumeGame(true);
}
...
function leaveGame(willRejoin) {
  if (!willRejoin && !g_IsObserver) resignGame(true);
  ...
  if (!g_IsReplay) Engine.AddReplayToCache(replayDirectory);

  if (g_IsController && Engine.HasXmppClient()) Engine.SendUnregisterGame();
...
function onTick() {
  ...
  if (g_Selection.dirty) {
    ...
    GetMultipleEntityStates(
      g_Selection.toList().filter(entId => !g_EntityStates[entId])
    );
  ...
  Engine.GetGUIObjectByName("resourcePop").textcolor =
    g_IsTrainingBlocked && now % 1000 < 500
...
function changeGameSpeed(speed) {
  if (!g_IsNetworked) Engine.SetSimRate(speed);
}

function updateIdleWorkerButton() {
  Engine.GetGUIObjectByName(
    "idleWorkerButton"
  ).enabled = Engine.GuiInterfaceCall("HasIdleUnits", {
...
function updateCinemaPath() {
  ...
  Engine.GetGUIObjectByName("session").hidden =
    !g_ShowGUI || isPlayingCinemaPath;
  Engine.Renderer_SetSilhouettesEnabled(
    !isPlayingCinemaPath &&
      Engine.ConfigDB_GetValue("user", "silhouettes") == "true"
  );
}
...
function updateGUIObjects() {
  ...
  if (g_ShowGuarding || g_ShowGuarded) updateAdditionalHighlight();
  ...
  if (g_ViewedPlayer > 0) {
    ...
    Engine.GetGUIObjectByName("devControlAll").checked =
  ...
  if (!g_IsObserver) {
    ...
    if (battleState) global.music.setState(global.music.states[battleState]);
  }
...
function onReplayFinished() {
  ...
  messageBox(
    ...
    translateWithContext(
      "replayFinished",
      "The replay has finished. Do you want to quit?"
    ),
    translateWithContext("replayFinished", "Confirmation"),
    [
      translateWithContext("replayFinished", "No"),
      translateWithContext("replayFinished", "Yes")
    ],
...
function updatePanelEntities() {
  let panelEnts =
    g_ViewedPlayer == -1
      ? GetSimState().players.reduce(
          (ents, pState) => ents.concat(pState.panelEntities),
          []
        )
      : GetSimState().players[g_ViewedPlayer].panelEntities;

  g_PanelEntities = g_PanelEntities.filter(panelEnt =>
    panelEnts.find(ent => ent == panelEnt.ent)
  );
  ...
  let panelEntIndex = ent =>
    g_PanelEntityOrder.findIndex(
      entClass => GetEntityState(ent).identity.classes.indexOf(entClass) != -1
    );

  g_PanelEntities = g_PanelEntities.sort(
    (panelEntA, panelEntB) =>
...
function createPanelEntityTooltip(panelEntState, template) {
  let getPanelEntNameTooltip = panelEntState =>
  ...
  return [
    ...
    .map(tooltip => tooltip(panelEntState))
    .filter(tip => tip)
    .join("\n");
}
...
function displayPanelEntities() {
  ...
  buttons.forEach((button, slot) => {
    if (
      button.hidden ||
      g_PanelEntities.some(ent => ent.slot !== undefined && ent.slot == slot)
    )
      return;
  ...
  for (
    let displayIndex = 0;
    displayIndex < Math.min(g_PanelEntities.length, buttons.length);
    ++displayIndex
  ) {
    ...
    let slot =
      panelEnt.slot === undefined
        ? buttons.findIndex(button => button.hidden)
        : panelEnt.slot;

    let panelEntButton = Engine.GetGUIObjectByName(
      "panelEntityButton[" + slot + "]"
    );
    panelEntButton.tooltip = panelEnt.tooltip;

    updateGUIStatusBar(
      "panelEntityHealthBar[" + slot + "]",
      panelEnt.currentHitpoints,
      panelEnt.maxHitpoints
    );

    if (panelEnt.slot === undefined) {
      let panelEntImage = Engine.GetGUIObjectByName(
        "panelEntityImage[" + slot + "]"
      );
...
function updateGroups() {
  ...
  for (let i in Engine.GetGUIObjectByName("unitGroupPanel").children) {
    ...
    button.onpress = (function(i) {
      return function() {
        performGroup(
          Engine.HotkeyIsPressed("selection.add") ? "add" : "select",
          i
        );
      };
    })(i);
    button.ondoublepress = (function(i) {
      return function() {
        performGroup("snap", i);
      };
    })(i);
    button.onpressright = (function(i) {
      return function() {
        performGroup("breakUp", i);
      };
    })(i);

    // Chose icon of the most common template (or the most costly if it's not unique)
    if (g_Groups.groups[i].getTotalCount() > 0) {
      let icon = GetTemplateData(
        GetEntityState(
          g_Groups.groups[i].getEntsGrouped().reduce((pre, cur) => {
            if (pre.ents.length == cur.ents.length)
              return getCostSum(pre.ents[0]) > getCostSum(cur.ents[0])
                ? pre
                : cur;
      ...
      Engine.GetGUIObjectByName("unitGroupIcon[" + i + "]").sprite = icon
...
function getAllyStatTooltip(resource, playerStates, sort) {
  ...
  for (let player in playerStates)
    tooltip.push({
      playername:
        colorizePlayernameHelper("■", player) + " " + g_Players[player].name,
      statValue:
        resource == "pop"
          ? sprintf(
              translate("%(popCount)s/%(popLimit)s/%(popMax)s"),
              playerStates[player]
            )
          : Math.round(playerStates[player].resourceCounts[resource]),
      orderValue:
        resource == "pop"
          ? playerStates[player].popCount
          : Math.round(playerStates[player].resourceCounts[resource])
    });
  if (sort) tooltip.sort((a, b) => sort * (b.orderValue - a.orderValue));
  return (
    "\n" +
    tooltip
      .map(stat => sprintf(translate("%(playername)s: %(statValue)s"), stat))
      .join("\n")
  );
}
...
function updatePlayerDisplay() {
  ...
  if (!viewedPlayerState) return;

  let tooltipSort = +Engine.ConfigDB_GetValue(
    "user",
    "gui.session.respoptooltipsort"
  );

  let orderHotkeyTooltip =
    Object.keys(viewablePlayerStates).length <= 1
      ? ""
      : "\n" +
  ...
  Engine.GetGUIObjectByName("resourcePop").caption = sprintf(
    translate("%(popCount)s/%(popLimit)s"),
    viewedPlayerState
  );
  Engine.GetGUIObjectByName("population").tooltip =
    translate("Population (current / limit)") +
    "\n" +
    sprintf(translate("Maximum population: %(popCap)s"), {
      popCap: viewedPlayerState.popMax
    }) +
    orderHotkeyTooltip +
...
function selectAndMoveTo(ent) {
  let entState = GetEntityState(ent);
  if (!entState || !entState.position) return;
...
function updateResearchDisplay() {
  let researchStarted = Engine.GuiInterfaceCall(
    "GetStartedResearch",
    g_ViewedPlayer
  );

  // Set up initial positioning.
  let buttonSideLength = Engine.GetGUIObjectByName("researchStartedButton[0]")
    .size.right;
  for (let i = 0; i < 10; ++i) {
  ...
  for (let tech in researchStarted) {
    ...
    if (numButtons >= 10) break;

    let template = GetTechnologyData(tech, g_Players[g_ViewedPlayer].civ);
    let button = Engine.GetGUIObjectByName(
      "researchStartedButton[" + numButtons + "]"
    );
    button.hidden = false;
    ...
    })(researchStarted[tech].researcher);

    let icon = "stretched:session/portraits/" + template.icon;
    Engine.GetGUIObjectByName(
      "researchStartedIcon[" + numButtons + "]"
    ).sprite = icon;

    // Scale the progress indicator.
    let size = Engine.GetGUIObjectByName(
      "researchStartedProgressSlider[" + numButtons + "]"
    ).size;

    // Buttons are assumed to be square, so left/right offsets can be used for top/bottom.
    size.top =
      size.left +
      Math.round(researchStarted[tech].progress * (size.right - size.left));
    Engine.GetGUIObjectByName(
      "researchStartedProgressSlider[" + numButtons + "]"
    ).size = size;

    Engine.GetGUIObjectByName(
      "researchStartedTimeRemaining[" + numButtons + "]"
    ).caption = Engine.FormatMillisecondsIntoDateStringGMT(
      researchStarted[tech].timeRemaining,
      translateWithContext("countdown format", "m:ss")
    );
  ...
  for (let type of ["Attack", "Auras", "Heal"])
    Engine.GuiInterfaceCall("EnableVisualRangeOverlayType", {
      ...
      enabled:
        Engine.ConfigDB_GetValue(
          "user",
          "gui.session." + type.toLowerCase() + "range"
        ) == "true"
    });
}
...
function appendSessionCounters(counters) {
  ...
  if (
    Engine.ConfigDB_GetValue("user", "gui.session.timeelapsedcounter") ===
    "true"
  ) {
    ...
    if (currentSpeed != 1.0)
      ...
      counters.push(
        sprintf(translate("%(time)s (%(speed)sx)"), {
          time: timeToString(simState.timeElapsed),
          speed: Engine.FormatDecimalNumberIntoString(currentSpeed)
        })
      );
    else counters.push(timeToString(simState.timeElapsed));
  }

  if (
    simState.ceasefireActive &&
    Engine.ConfigDB_GetValue("user", "gui.session.ceasefirecounter") === "true"
  )
