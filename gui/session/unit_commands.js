...
function setupUnitPanel(guiName, unitEntStates, playerState) {
  ...
  for (let i = 0; i < numberOfItems; ++i) {
    let data = {
      ...
      countDisplay: Engine.GetGUIObjectByName(
        "unit" + guiName + "Count[" + i + "]"
      ),
      totAmount: Engine.GetGUIObjectByName("unit" + guiName + "Stat[" + i + "]")
    };
    ...
    if (
      g_SelectionPanels[guiName].setupButton &&
      !g_SelectionPanels[guiName].setupButton(data)
    )
      continue;

    // TODO: we should require all entities to have icons, so this case never occurs
    if (data.icon && !data.icon.sprite) data.icon.sprite = "BackgroundBlack";
  }
...
function updateUnitCommands(
  ...
  for (let panel in g_SelectionPanels) g_SelectionPanels[panel].used = false;
  ...
  if (
    ...
    for (let guiName of g_PanelsOrder) {
      if (
        ...
        g_SelectionPanels[guiName].conflictsWith.some(
          p => g_SelectionPanels[p].used
        )
      )
        continue;
  ...
  for (let panelName in g_SelectionPanels)
    Engine.GetGUIObjectByName(
      "unit" + panelName + "Panel"
    ).hidden = !g_SelectionPanels[panelName].used;
}
...
function getAllBuildableEntities(selection) {
  return Engine.GuiInterfaceCall("GetAllBuildableEntities", {
    entities: selection
  });
}
...
function getNumberOfRightPanelButtons() {
  ...
  for (let prop of ["Construction", "Training", "Pack", "Gate", "Upgrade"])
    if (g_SelectionPanels[prop].used) sum += g_unitPanelButtons[prop];
...
function getAllConsumingProducts(selection) {
  return Engine.GuiInterfaceCall("GetAllConsumingProducts", {
    entities: selection
  });
}

function getAllConsumingProductsFromSelection() {
  if (!g_allConsumingProducts)
    g_allConsumingProducts = getAllConsumingProducts(g_Selection.toList());

  return g_allConsumingProducts;
}
