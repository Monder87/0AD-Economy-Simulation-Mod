...
function updateGarrisonHealthBar(entState, selection) {
  if (!entState.garrisonHolder) return;
  ...
  if (totalGarrisonHealth > 0) {
    ...
    healthSize.rtop =
      100 -
      100 * Math.max(0, Math.min(1, totalGarrisonHealth / maxGarrisonHealth));
...
function displaySingle(entState) {
  ...
  if (genericName && template.pack && template.pack.state == "packed")
    genericName = sprintf(translate("%(genericName)s — Packed"), {
  ...
  let playerColor = rgbToGuiColor(
    g_DisplayedPlayerColors[entState.player],
    128
  );

  // Indicate disconnected players by prefixing their name
  if (g_Players[entState.player].offline)
    playerName = sprintf(translate("\\[OFFLINE] %(player)s"), {
      player: playerName
    });
  ...
  if (
    entState.identity &&
    entState.identity.rank &&
    entState.identity.classes
  ) {
    Engine.GetGUIObjectByName("rankIcon").tooltip = sprintf(
      translate("%(rank)s Rank"),
      {
        rank: translateWithContext("Rank", entState.identity.rank)
      }
    );
    Engine.GetGUIObjectByName("rankIcon").sprite =
      "stretched:session/icons/ranks/" + entState.identity.rank + ".png";
  ...
  let sectionPosTopBotton = Engine.GetGUIObjectByName("sectionPosTopBotton");
  ...
  if (showHealth) {
    ...
    healthSize.rright =
      100 *
      Math.max(0, Math.min(1, entState.hitpoints / entState.maxHitpoints));
    unitHealthBar.size = healthSize;
    Engine.GetGUIObjectByName("healthStats").caption = sprintf(
      translate("%(hitpoints)s / %(maxHitpoints)s"),
      {
    ...
    wealthSection.size = sectionPosTopBotton.size;
    captureSection.size = showResource
      ? sectionPosMiddle.size
      : sectionPosBottom.size;
    resourceSection.size = showResource
      ? sectionPosBottom.size
      : sectionPosMiddle.size;
  } else {
  ...
  wealthSection.hidden = !showWealth;
  if (showWealth) {
    let unitWealthBar = Engine.GetGUIObjectByName("wealthBar");
    let wealthSize = unitWealthBar.size;
    wealthSize.rright =
      100 *
      Math.max(0, Math.min(1, entState.balance / entState.walletMaxCapacity));
    unitWealthBar.size = wealthSize;
    Engine.GetGUIObjectByName("wealthStats").caption = sprintf(
      translate("%(balance)s / %(walletMaxCapacity)s"),
      {
        balance: Math.ceil(entState.balance),
        walletMaxCapacity: Math.ceil(entState.walletMaxCapacity)
      }
    );
  }
  ...
  if (entState.capturePoints) {
    let setCaptureBarPart = function(playerID, startSize) {
      let unitCaptureBar = Engine.GetGUIObjectByName(
        "captureBar[" + playerID + "]"
      );
      ...
      let size =
        ...
        Math.max(
          ...
          Math.min(
            1,
            entState.capturePoints[playerID] / entState.maxCapturePoints
          )
        );
      sizeObj.rright = startSize + size;
      unitCaptureBar.size = sizeObj;
      unitCaptureBar.sprite =
        "color:" + rgbToGuiColor(g_DisplayedPlayerColors[playerID], 128);
    ...
    for (let i in entState.capturePoints)
      if (i != entState.player) size = setCaptureBarPart(i, size);

    let captureText = sprintf(
      translate("%(capturePoints)s / %(maxCapturePoints)s"),
      {
    ...
    Engine.GetGUIObjectByName("captureStats").caption = showSmallCapture
      ? ""
      : captureText;
    Engine.GetGUIObjectByName("capture").tooltip = showSmallCapture
      ? captureText
      : "";
  }
  ...
  if (entState.promotion) {
    ...
    experienceSize.rtop =
      ...
      100 *
        Math.max(
          0,
          Math.min(1, 1.0 * +entState.promotion.curr / +entState.promotion.req)
        );
    experienceBar.size = experienceSize;

    if (entState.promotion.curr < entState.promotion.req)
      Engine.GetGUIObjectByName("experience").tooltip = sprintf(
        translate("%(experience)s %(current)s / %(required)s"),
        {
          experience:
            '[font="sans-bold-13"]' + translate("Experience:") + "[/font]",
      ...
      Engine.GetGUIObjectByName("experience").tooltip = sprintf(
        translate("%(experience)s %(current)s"),
        {
          experience:
            '[font="sans-bold-13"]' + translate("Experience:") + "[/font]",
          current: Math.floor(entState.promotion.curr)
  ...
  if (entState.resourceSupply) {
    let resources = entState.resourceSupply.isInfinite
      ? translate("∞") // Infinity symbol
    ...
    resourceSize.rright = entState.resourceSupply.isInfinite
      ? 100
      : 100 *
        Math.max(
          0,
          Math.min(
            1,
            +entState.resourceSupply.amount / +entState.resourceSupply.max
          )
        );
    unitResourceBar.size = resourceSize;

    Engine.GetGUIObjectByName("resourceLabel").caption = sprintf(
      translate("%(resource)s:"),
      {
  ...
  if (entState.resourceCarrying && entState.resourceCarrying.length) {
    ...
    resourceCarryingIcon.sprite =
      "stretched:session/icons/resources/" + carried.type + ".png";
    ...
    resourceCarryingIcon.sprite =
      "stretched:session/icons/resources/" +
      entState.trader.goods.type +
      ".png";
    ...
    resourceCarryingText.caption = entState.foundation.numBuilders
      ? Engine.FormatMillisecondsIntoDateStringGMT(
          entState.foundation.buildTime.timeRemaining * 1000,
          translateWithContext("countdown format", "m:ss")
        )
      : "";
  } else if (
    entState.resourceSupply &&
    (!entState.resourceSupply.killBeforeGather || !entState.hitpoints)
  ) {
    resourceCarryingIcon.sprite = "stretched:session/icons/repair.png";
    resourceCarryingText.caption = sprintf(translate("%(amount)s / %(max)s"), {
    ...
    Engine.GetGUIObjectByName("resourceCarryingIcon").tooltip = translate(
      "Current/max gatherers"
    );
    ...
    resourceCarryingText.caption = entState.repairable.numBuilders
      ? Engine.FormatMillisecondsIntoDateStringGMT(
          entState.repairable.buildTime.timeRemaining * 1000,
          translateWithContext("countdown format", "m:ss")
        )
      : "";
  } else {
  ...
  Engine.GetGUIObjectByName("iconBorder").tooltip = iconTooltips
    .filter(tip => tip)
    .join("\n");
...
function displayMultiple(entStates) {
  ...
  for (let entState of entStates) {
    ...
    if (entState.capturePoints) {
      ...
      capturePoints = entState.capturePoints.map(
        (v, i) => v + capturePoints[i]
      );
    }
  ...
  if (averageHealth > 0) {
    ...
    let healthSize = unitHealthBar.size;
    healthSize.rtop =
      100 - 100 * Math.max(0, Math.min(1, averageHealth / maxHealth));
    unitHealthBar.size = healthSize;

    Engine.GetGUIObjectByName(
      "healthMultiple"
    ).tooltip = getCurrentHealthTooltip({
  ...
  if (maxCapturePoints > 0) {
    let setCaptureBarPart = function(pID, startSize) {
      let unitCaptureBar = Engine.GetGUIObjectByName(
        "captureBarMultiple[" + pID + "]"
      );
      let sizeObj = unitCaptureBar.size;
      sizeObj.rtop = startSize;

      let size =
        100 * Math.max(0, Math.min(1, capturePoints[pID] / maxCapturePoints));
      sizeObj.rbottom = startSize + size;
      unitCaptureBar.size = sizeObj;
      unitCaptureBar.sprite =
        "color:" + rgbToGuiColor(g_DisplayedPlayerColors[pID], 128);
      unitCaptureBar.hidden = false;
      return startSize + size;
    };
    ...
    for (let i in capturePoints)
      if (i != playerID) size = setCaptureBarPart(i, size);
  ...
  if (Object.keys(totalCarrying).length)
    numberOfUnits.tooltip = sprintf(translate("%(label)s %(details)s\n"), {
      ...
      details: bodyFont(
        Object.keys(totalCarrying)
          .filter(res => totalCarrying[res] != 0)
          .map(res =>
            sprintf(translate("%(type)s %(amount)s"), {
              type: resourceIcon(res),
              amount: totalCarrying[res]
            })
          )
          .join("  ")
      )
  ...
  if (Object.keys(totalLoot).length)
    numberOfUnits.tooltip += sprintf(translate("%(label)s %(details)s"), {
      ...
      details: bodyFont(
        Object.keys(totalLoot)
          .filter(res => totalLoot[res] != 0)
          .map(res =>
            sprintf(translate("%(type)s %(amount)s"), {
              type: resourceIcon(res),
              amount: totalLoot[res]
            })
          )
          .join("  ")
      )
...
function updateSelectionDetails() {
  let supplementalDetailsPanel = Engine.GetGUIObjectByName(
    "supplementalSelectionDetails"
  );
  ...
  if (entStates.length == 1) displaySingle(entStates[0]);
  else displayMultiple(entStates);
  ...
  if (
    ...
    !Engine.GetGUIObjectByName("unitGarrisonPanel").hidden
  )
