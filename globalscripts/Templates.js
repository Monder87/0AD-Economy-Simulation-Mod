...
function loadCivFiles(selectableOnly) {
  ...
  for (let filename of Engine.ListDirectoryFiles(
...
function GetIdentityClasses(template) {
  ...
  if (template.Rank) classList = classList.concat(template.Rank);
  return classList;
}
...
function MatchesClassList(classes, match) {
  ...
  for (let sublist of match) {
    ...
    if (
      sublist.every(
        c =>
          (c[0] == "!" && classes.indexOf(c.substr(1)) == -1) ||
 ...
 */
function GetModifiedTemplateDataValue(
  template,
  value_path,
  mod_key,
  player,
  ...
  if (player)
    current_value = ApplyValueModificationsToTemplate(
      mod_key,
      current_value,
      player,
      template
    ...
    current_value = GetTechModifiedProperty(
      modifiers,
      GetIdentityClasses(template.Identity),
      mod_key,
      current_value
    );
 ...
 *                             id provided.
 */
function GetTemplateDataHelper(
  template,
  player,
  auraTemplates,
  resources,
  ...
  let getEntityValue = function(value_path, mod_key) {
    return GetModifiedTemplateDataValue(
      template,
      value_path,
      mod_key,
      player,
  ...
  if (template.Attack) {
    ...
    for (let type in template.Attack) {
      ...
      if (type == "Capture")
        ...
        ret.attack[type].elevationAdaptedRange = Math.sqrt(
          ret.attack[type].maxRange *
      ...
      if (template.Attack[type].Splash) {
        ...
        for (let damageType of damageTypes.GetTypes())
          ret.attack[type].splash[damageType] = getAttackStat(
            "Splash/" + damageType
          );
      }
    }
  ...
  if (template.BuildingAI)
    ret.buildingAI = {
      defaultArrowCount: Math.round(
        getEntityValue("BuildingAI/DefaultArrowCount")
      ),
      garrisonArrowMultiplier: getEntityValue(
        "BuildingAI/GarrisonArrowMultiplier"
      ),
  ...
  if (template.BuildRestrictions) {
    ...
    if (template.BuildRestrictions.Distance) {
      ...
      if (template.BuildRestrictions.Distance.MinDistance)
        ret.buildRestrictions.distance.min = getEntityValue(
          "BuildRestrictions/Distance/MinDistance"
        );

      if (template.BuildRestrictions.Distance.MaxDistance)
        ret.buildRestrictions.distance.max = getEntityValue(
          "BuildRestrictions/Distance/MaxDistance"
        );
    }
  }
  ...
  if (template.Footprint) {
    ...
    else warn("GetTemplateDataHelper(): Unrecognized Footprint type");
  }
  ...
  if (template.ResourceGatherer) {
    ...
    for (let type in template.ResourceGatherer.Rates)
      ret.resourceGatherRates[type] =
        getEntityValue("ResourceGatherer/Rates/" + type) * baseSpeed;
  }
  ...
  if (template.ResourceTrickle) {
    ...
    for (let type in template.ResourceTrickle.Rates)
      ret.resourceTrickle.rates[type] = getEntityValue(
        "ResourceTrickle/Rates/" + type
      );
  }
  ...
  if (template.Obstruction) {
    ret.obstruction = {
      ...
      disableBlockMovement:
        "" + template.Obstruction.DisableBlockMovement == "true",
      disableBlockPathfinding:
        "" + template.Obstruction.DisableBlockPathfinding == "true",
      shape: {}
    };
    ...
    } else ret.obstruction.shape.type = "cluster";
  }
  ...
  if (template.Health) ret.health = Math.round(getEntityValue("Health/Max"));
    ...
    for (let upgradeName in template.Upgrade) {
      ...
      if (upgrade.Cost)
        for (let res in upgrade.Cost)
          cost[res] = getEntityValue(
            "Upgrade/" + upgradeName + "/Cost/" + res,
            "Upgrade/Cost/" + res
          );
      if (upgrade.Time)
        cost.time = getEntityValue(
          "Upgrade/" + upgradeName + "/Time",
          "Upgrade/Time"
        );
  ...
  if (template.ProductionQueue) {
    ...
    for (let res in template.ProductionQueue.TechCostMultiplier)
      ret.techCostMultiplier[res] = getEntityValue(
  ...
  if (template.EntityFinance) {
    ret.entityFinance = {
      WalletCapacities: getEntityValue("EntityFinance/WalletCapacities"),
      EarningRate: getEntityValue("EntityFinance/EarningRate")
    };
  }
  ...
  if (template.WallSet) {
    ret.wallSet = {
      templates: {
        ...
        fort:
          template.WallSet.Templates.Fort ||
          "structures/" + template.Identity.Civ + "_fortress",
...
function GetTechnologyDataHelper(template, civ, resources) {
  ...
  if (template.specificName)
    ret.name.specific =
      template.specificName[civ] || template.specificName.generic;
